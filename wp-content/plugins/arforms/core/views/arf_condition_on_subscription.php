...
class ARF_condition_on_subscription {
    ...
    function arf_condition_on_subscription_html_fun($id, $is_ref_form, $values) {
        ...
        global $arfieldhelper, $maincontroller, $conditional_logic_array_if;
        ?> 
                                ...
                                <div class="arf_condition_on_subscription_logic_div" id="arf_rule_condition_on_subscription_<?php echo $rule_i; ?>">
                                    ...
                                    <div style="<?php echo (is_rtl()) ? 'float: right;' : 'float: left;';?> width: 100%">
                                        <span id="select_ar_condition_on_subscription_field">
                                            <div class="sltstandard" style="width: 100%;">
                                                ...
                                                <?php
                                                    ...
                                                    $arf_subscription_condition_attr = array('class' => 'arf_condition_on_subscription_field_action');

                                                    echo $maincontroller->arf_selectpicker_dom( 'options[arf_condition_on_subscription_rules]['. $rule_i .'[field_id]', 'arf_subscription_condition_field_'. $rule_i , 'arf_name_field_dropdown', 'width:100%;', $condition_value['field_id'], $arf_subscription_condition_attr, $selectbox_field_options, false, array(), false, $selectbox_field_options_attr, false, array(), true );
                                                ?>
                                            </div>
                                        </span>
                                        ...
                                        <span id="select_ar_conditional_redirect_operator">
                                            <div class="sltstandard" style="width:100%">
                                                <?php echo $arfieldhelper->arf_cl_rule_menu_for_conditional_subscription('arf_subscription_condition_operator_' . $rule_i, 'arf_subscription_condition_operator_' . $rule_i, $condition_value['operator'], $rule_i, $condition_value['field_type']); ?>
                                            </div>
                                        </span>

                                        <span id="select_ar_conditional_redirect_value">
                                            <input style="width:100%;" type="text" class="txtstandardnew arf_large_input_box" placeholder="<?php echo (esc_attr($condition_value['field_type']) == "date") ? 'YYYY/MM/DD':'' ; ?>" value="<?php if(isset($condition_value['value'])){echo $condition_value['value'];} ?>" id="arf_subscription_condition_field_value_<?php echo $rule_i; ?>" onkeyup="this.setAttribute('value',this.value)" name="options[arf_condition_on_subscription_rules][<?php echo $rule_i; ?>][value]">
                                        </span>
        ...
        <div class="arf_condition_on_subscription_field_dropdown arf_name_field_dropdown" id="arf_condition_on_subscription_dropdown_html" style="display: none;">
            ...
            if (!empty($values['fields'])) {
                foreach ($values['fields'] as $val_key => $fo) {
                    if ($fo['type'] != 'divider' && $fo['type'] != 'section' && $fo['type'] != 'break' && $fo['type'] != 'captcha' && $fo['type'] != 'html' && $fo['type'] != 'file' && $fo['type'] != 'arf_repeater' && $fo['type'] != 'time' && !isset( $fo['has_parent'] ) ) {
            ...
            <div style="float: left; width: 100%">
                ...
                <span id="select_ar_condition_on_subscription_field" style="width:170px; float: left;">
                    <div class="sltstandard">
                        <?php
                            $arf_cond_subs_opts = array('' => addslashes(esc_html__('Select Field', 'ARForms')));
                            echo $maincontroller->arf_selectpicker_dom( 'arf_subscription_condition_field_'.$rule_i, 'arf_subscription_condition_field_'.$rule_i, '', 'width:160px;', '', array(), $arf_cond_subs_opts, false, array(), false, array(), false, array(), true );
                        ?>
                    </div>
                </span>
    ...
    function arf_importtime_update_condition_on_subscription_field_id($option_arr_new, $old_new_field_id, $form_id) {
        global $wpdb,$MdlDb;
        if (isset($option_arr_new['arf_condition_on_subscription_rules']) && is_array($option_arr_new['arf_condition_on_subscription_rules']) && count($option_arr_new['arf_condition_on_subscription_rules']) > 0 ) {
            $arf_condition_on_subscription_rules = array();
            if (!empty($option_arr_new['arf_condition_on_subscription_rules'])) {
                foreach ($option_arr_new['arf_condition_on_subscription_rules'] as $new_rule) {
                    ...
                    $cl_subscription_id = isset( $old_new_field_id[$new_rule['field_id']] ) ? $old_new_field_id[$new_rule['field_id']] : '';
