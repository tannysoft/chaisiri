...
class arf_submit_confirmation_summary{
		...
		<div class="arf_confirmation_summary_container">
			<div class="arf_confirmation_summary_inner_container">
				...
				<div class="arf_confirmation_summary_inner_block arfmarginl15" style="<?php echo $arf_enable_confirmation_summary; ?>">
					...
					<div class="arf_confirmation_summary_input_wrapper" id="arf_confirmation_summary_hide_html_wrapper">
						<label class="arf_dropdown_autoresponder_label"><?php echo addslashes(esc_html__('Hide HTML Field From Summary','ARForms')); ?></label>

						<div class="arf_custom_checkbox_div" style="margin-top: 7px;">
							<div class="arf_custom_checkbox_wrapper">
								<input type="checkbox" name="options[arf_confirmation_summary_html_field]" id="arf_confirmation_summary_hide_html_field" <?php (isset($values['arf_confirmation_summary_html_field'])) ? checked($values['arf_confirmation_summary_html_field'],1) : ''; ?> value="1" />
								<svg width="18px" height="18px">
									<?php echo ARF_CUSTOM_UNCHECKED_ICON; ?>
									<?php echo ARF_CUSTOM_CHECKED_ICON; ?>
								</svg>
							...
							<span>
								<label for="arf_confirmation_summary_hide_html_field"><?php esc_html_e('Yes','ARForms'); ?></label>
							</span>
						</div>
					</div>
				</div>
	...
	function arf_save_confirmation_summary($options,$values){
		...
		$options['arf_confirmation_summary_html_field'] = isset($values['options']['arf_confirmation_summary_html_field']) ? $values['options']['arf_confirmation_summary_html_field'] : '';
	...
	function arf_add_confirmation_summary_box_outside($arf_form, $form, $form_data_id,$arfbrowser_name,$browser_info){
		...
		if( $inputStyle == 'standard' ){
			...
			$wrapper_class = 'arf_material_outline_form';
		}
		...
		$arfsubmitbuttonstyle = str_replace( ' ', '_', $arfsubmitbuttonstyle );

        if( $summary_display_position == 'before' ){
			$submit_btn_content .= '<button class="arf_submit_btn arf_submit_btn_'.$arfsubmitbuttonstyle.' '.$sbmt_class.' btn-info arf_submit_after_confirm arfstyle-button ' . $submitbtnclass .' '.$arfbrowser_name.'"  id="arf_submit_btn_' . $form_data_id . '_confirm" name="arf_submit_btn_' . $form_data_id . '" data-style="zoom-in" >';
        	...
        	if( $arf_allow_print ){
        		$submit_btn_content .= "<button type='button' class='arf_submit_btn arf_submit_btn_{$arfsubmitbuttonstyle} {$sbmt_class} arf_print_summary' data-home-url='".home_url()."' data-form-unique-id='{$form_data_id}'>";
						...
						function arf_cf_print_data(){
							...
							if( total_rows > 0 ){
								for(var tr = 0; tr < total_rows; tr++ ){
									...
									if( current_tr.querySelector('.arf_confirmation_summary_label_full_width') != null ){
									...
									} else if( current_tr.querySelector('.arf_confirmation_summary_repeater_label') != null ){

										var td_center = document.createElement('td');
										td_center.setAttribute('colspan','2');
										current_tr.setAttribute('rowspan','2');

										td_center.innerHTML = current_tr.querySelector('.arf_confirmation_summary_repeater_label').innerHTML;
										tr_container.appendChild(td_center);

									} else {

										var td_left = document.createElement('td');
										td_left.style.wordBreak = "break-all";
										...
										td_right.style.wordBreak = "break-all";
								...
								if( doc.querySelectorAll('.arf_confirmation_summary_repeater_input') != null ){

									var arf_repeater_inputs = doc.querySelectorAll('.arf_repeater_label_td');

									var repeater_rows = arf_repeater_inputs.length;

									if( repeater_rows > 0 ){

										for (var rp_tr = 0; rp_tr < repeater_rows; rp_tr++) {
										
											var current_rp_tr = arf_repeater_inputs[rp_tr];

											var rp_tr_container = document.createElement('tr');
											var rp_tr_classList = ( parseInt(rp_tr) % 2 == 0 ) ? 'arf_confirmation_summary_row arf_confirmation_summary_even_row' : 'arf_confirmation_summary_row arf_confirmation_summary_odd_row';
											rp_tr_container.setAttribute('class',rp_tr_classList);

											var rp_td_left = document.createElement('td');
											rp_td_left.style.wordBreak = "break-all";

											rp_td_left.innerHTML = current_rp_tr.innerHTML;
											rp_tr_container.appendChild(rp_td_left);

											var rp_td_right = document.createElement('td');
											rp_td_right.style.wordBreak = "break-all";

											rp_td_right.innerHTML = current_rp_tr.nextElementSibling.innerHTML;

											rp_tr_container.appendChild(rp_td_right);

											table_wrapper.appendChild(rp_tr_container);
										}
									}
								}
							}
