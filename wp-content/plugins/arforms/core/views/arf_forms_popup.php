...
$frm_list = array( '0' => esc_html__('Please select form','ARForms') );

$res = $arfform->arf_select_db_data(true, '', $MdlDb->forms, 'id,name,is_template,status', 'WHERE is_template = %d AND status = %s', array( 0, 'published' ) );

if(!empty($res)){
	$res_count = count($res);
	foreach ($res as $key => $value) {
		$frm_list[$value->id] = $value->name." (id: ".$value->id.")";
	}
}
            			...
            			<form method="get" id="list_popup_form" class="arf_list_popup_form" onsubmit="return apply_bulk_delete_popup();" style="float:left;width:98%;">
		            		...
		            		<div class="alignleft actions">
		                        <div class="arf_list_bulk_action_wrapper">
		                            <?php
		                            	$bulk_action_opts = array( '-1' => addslashes(esc_html__('Bulk Actions', 'ARForms')) );

		                            	foreach ($actions as $name => $title) {
		                                    $opt_class = 'edit' == $name ? ' hide-if-no-js ' : '';

		                                    $bulk_action_opts[$name] = $title;
		                                }

		                            	echo $maincontroller->arf_selectpicker_dom( 'action1', 'arf_bulk_action_one', '', '', '-1', array(), $bulk_action_opts, false,  $opt_class);
		                            ?>
		                        </div>
		                    ...
		                    <table cellpadding="0" cellspacing="0" border="0" class="display table_grid arf_popup_list_table" id="example">
		                    	<thead>
                                    <tr>
                                        ...
                                        <th style="width:10%;"><?php echo addslashes(esc_html__('Form ID', 'ARForms')); ?></th>
                                        <th style="width:35%;"><?php echo addslashes(esc_html__('Form Title', 'ARForms')); ?></th>
                                        <th style="width:25%;"><?php echo esc_html__('Popup Type', 'ARForms'); ?></th>
                                        <th style="width:10%;"><?php echo esc_html__('Status', 'ARForms'); ?></th>
		                    ...
		                    <div class="alignleft actions">
		                        <div class="arf_list_bulk_action_wrapper">
		                            <?php
		                            	...
		                            	echo $maincontroller->arf_selectpicker_dom( 'action3', 'arf_bulk_action_two', '', '', '-1', array(), $bulk_action_opts, false,  $opt_class);
		                            ?>
		                        </div>
			...
			<div id="arf_display_form_in_popup_model" class="arf_popup_container arf_popup_container_other_option_model">
		        ...
		        <div class="arf_popup_content_container" id="arf_popup_content_container">
		            ...
		            <div class="dt_dl arf_form_list_dd" style="<?php if (is_rtl()) { echo 'text-align:right;'; } else { echo 'text-align:left;'; } ?>">
	                  	<?php
	                  		echo $maincontroller->arf_selectpicker_dom( 'arf_popup_selected_form', 'arf_popup_selected_form', 'arf_popup_selected_form', 'width:300px;', '', array(), $frm_list, false, array(), false, array(), false, array(), true );
	                  	?>
	              	</div>
	              	<br/>
	              	<br/>
		            ...
		            <div class="arf_popup_display_container">
					    ...
					    <div class="arf_popup_section_row site_wide_popup_sub_options" style="margin-bottom: 20px;">
							<label class="arf_popup_label_left">
		                		<span><?php esc_html_e('Logged in User', 'ARForms'); ?></span>
		                	</label>
		                	<div class="arf_custom_checkbox_wrapper">
