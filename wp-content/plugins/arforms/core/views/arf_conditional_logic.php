<?php
global $arfieldhelper,$conditional_logic_array_if,$conditional_logic_array_than,$maincontroller;
    ...
    if (!empty($all_hidden_fields)) {
        ...
        foreach($all_hidden_fields as $val_key => $fo) {
            ...
            if (!in_array($fo['type'], $conditional_logic_array_than)) {
                ...
                if($current_field_id !="" && $arfieldhelper->arf_execute_function($display_name,'strip_tags') ==""){
                    $conditional_logic_field_options .= '<li class="arf_selectbox_option arfhidden" data-value="' . $current_field_id . '" data-type="' . $fo["type"] . '" data-label="[Field Id:'.$current_field_id.']">[Field Id:'.$current_field_id.']</li>';
                }else{
                    $conditional_logic_field_options .= '<li class="arf_selectbox_option arfhidden" data-value="' . $current_field_id . '" data-type="' . $fo["type"] . '" data-label="' . $arfieldhelper->arf_execute_function($display_name,'strip_tags') . '">' . $arfieldhelper->arf_execute_function($display_name,'strip_tags') . '</li>';    
                }
                        ...
                        <div class="arftablerow arf_conditional_logic_div" style="margin-top:15px;width:100%; box-sizing: border-box;-webkit-box-sizing: border-box;-o-box-sizing: border-box;-moz-box-sizing: border-box;" id="arf_conditional_logic_div_<?php echo $rule_i; ?>">
                                    ...
                                    <div class="arfcommonconditionlogin">
                                        <span id="select_arf_conditional_logic_logical_operator">
                                            <div class="sltstandard" style="width:100%">
                                                <?php
                                                    $arf_logical_operator_val = ($logic_rules['logical_operator'] == 'and') ? 'and' : 'or';
                                                    $arf_logical_operator_opt = array(
                                                        'and' => addslashes(esc_html__('All', 'ARForms')),
                                                        'or' => addslashes(esc_html__('Any', 'ARForms'))
                                                    );
                                                    echo $maincontroller->arf_selectpicker_dom( 'options[arf_conditional_logic_rules]['. $rule_i .'][logical_operator]', 'arf_conditional_logic_logical_operator_'.$rule_i , '', 'width:100%;', $arf_logical_operator_val, array(), $arf_logical_operator_opt );
                                                ?>
                                            </div>
                                        </span>
                                    </div>
                            ...
                            <div class="arfcolumnright arf_new_conditional_logic" style="<?php echo (is_rtl()) ? 'padding-left: 5px;' : 'padding-right: 5px;'; ?>" id="arf_result_<?php echo $rule_i; ?>">
                                <?php foreach ($logic_rules['result'] as $result_i => $result) {
                                    ...
                                    <?php $arformcontroller->arf_result_add($rule_i, $result_i, $is_ajax, $values, count($logic_rules['result']), $all_hidden_fields); ?>
                                <?php } ?>
                            </div>
