...
class arf_mailerlite {
            ...
            <table class="wp-list-table widefat post " style="margin:20px 0 20px 10px; border:none;">
                ...
                <tr id="mailerlite_api_tr2" <?php if ($autores_type['mailerlite_type'] != 1) echo 'style="display:none;"'; ?>>
                    ...
                    <td style=" padding-top:3px; padding-bottom:3px; padding-left:5px; overflow: visible;"><span id="select_mailerlite">
                        ...
                        <div id="mailerlite_del_link" style="padding-left:5px; margin-top:10px; clear: both; <?php if ($mailerlite_data->is_verify == 0) { ?>display:none;<?php } ?>" class="arlinks">
                        ...
                        if (( $arfaction == 'new' || ( $arfaction == 'duplicate' and isset($arf_template_id) and $arf_template_id < 100 ) ) || (isset($mailerlite_arr['enable']) and $mailerlite_arr['enable'] == 0 )) {
                            ...
                            <div id="autores-mailerlite" class="autoresponder_inner_block" data-if="sadsa" >
                                ...
                                <div class="sltstandard">
                                    ...
                                    if( isset($res14['responder_list_id']) ){
                                    ...
                                    $mailerlite_enable_class = (isset($mailerlite_arr['enable']) && $mailerlite_arr['enable'] == 1) ? '' : 'arf_not_allowd_optins';

                                    if(isset($mailerlite_arr['enable']) && $mailerlite_arr['enable'] == 1){
                                        $mailerlite_opt_disable = false;
                                    }else{
                                        $mailerlite_opt_disable = true;
                                    }
				    
                				    $mailerlite_attr = array();
                				    if( $setvaltolic != 1 ){
                				    	$mailerlite_attr = array( 'readonly' => 'readonly' );
                				    }

                                    echo $maincontroller->arf_selectpicker_dom( 'i_mailerlite_list', 'i_mailerlite_list', $mailerlite_enable_class, 'width:170px;', $selected_list_id, $mailerlite_attr, $responder_list_option, false, array(), $mailerlite_opt_disable, array(), false, array(), true );
                                    ?>                                    
                                </div>
                            </div>
                            <?php
    ...
    function arf_mailerlite_after_create_entry($entry_id, $form_id) {
        ...
        $results = wp_cache_get('arf_mailer_result_'.$form_id);
        if( false == $results ){
            $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $MdlDb->forms . " WHERE id = %d", $form_id));
            wp_cache_set('arf_mailer_result_'.$form_id, $results);
        }
