            ...
            <div class="inside">
                <div class="frm_settings_form ">
                    ...
                    if (isset($_REQUEST['arf_import_btn']) && current_user_can('arfchangesettings')) {
                        ...
                        if( !isset( $_REQUEST['arf_nonce'] ) || ( isset( $_REQUEST['arf_nonce'] ) && !wp_verify_nonce( $_REQUEST['arf_nonce'], 'arf_import_form' ) ) ){
                        ?>
                            <script type="text/javascript" language="javascript"> setTimeout(function () {
                                ...
                                <div id="error_message" class="arf_error_message">
                                    <div class="message_descripiton">
                                        <div style="float: left; margin-right: 15px;" id=""><?php echo addslashes(esc_html__('Sorry, You are not an authorized person to perform this action.', 'ARForms')); ?></div>
                        ...
                        <?php
                            ...
                            if (isset($xml->form)) {
                                ...
                                foreach ($xml->children() as $key_main => $val_main) {
                                    ...
                                    if ($submit_bg_img != '') {
                                        ...
                                        $arfilecontroller->capabilities = array( 'arfchangesettings' );

                                        $arfilecontroller->check_nonce = true;
                                        $arfilecontroller->nonce_action = 'arf_import_form';
                                        $arfilecontroller->nonce_data = isset( $_REQUEST['arf_nonce'] ) ? $_REQUEST['arf_nonce'] : '';
                                    ...
                                    foreach ($val_main->general_options->children() as $key => $val) {
                                        if ($key == 'options') {
                                            ...
                                            $enable_confirm_summary_flag = $options_arr['arf_confirmation_summary'];
                                    ...
                                    $arf_skipped_addon_fields = array();
                                    ...
                                    foreach ($val_main->fields->children() as $key_fields => $val_fields) {
                                        ...
                                        foreach ($val_fields as $key_field => $val_field) {
                                            ...
                                            } elseif( $val_fields->type == 'break' && $key_field == 'field_options'){
                                                
                                                $arf_pagebreak_options = arf_json_decode(trim($val_field),true);

                                                $arf_pagebreak_type_options = isset($arf_pagebreak_options['page_break_wizard_theme']) ? $arf_pagebreak_options['page_break_wizard_theme'] : '';

                                                if ( $arf_pagebreak_type_options != '' ) {
                                                    $arf_page_break_wizard_theme_style = $arf_pagebreak_options['page_break_wizard_theme'];
                                                }
                                                $fields_option[$key_field] = trim(json_encode($arf_pagebreak_options));
                                        ...
                                        if( !in_array($val_fields->type,$outside_fields)){
                                            $skipped_field_options = arf_json_decode( $fields_option['field_options'], true );
                                            $arf_skipped_addon_fields[$new_field_id] = $skipped_field_options['inner_class'];
                                        }
                                    ...
                                    if (in_array('select', $type_array)) {
                                        foreach ($content_array as $key_type => $value_type) {
                                                ...
                                                foreach ( $new_id_array as $key=>$val ) {
                                                    if ( isset( $fleld_data_options['parent_field_id'] ) ) {
                                                        if ( $fleld_data_options['parent_field_id'] == $val['old_id']) {
                                    ...
                                    foreach ($new_field_order as $key => $value) {
                                        if(strpos($key, '_confirm') !== false) {
                                            ...
                                            if( !empty( $arf_skipped_addon_fields[$key] ) ){
                                                $final_field_order[$arf_skipped_addon_fields[$key].'|'.$value] = $value;
                                            } else {
                                    ...
                                    foreach( $new_inner_field_order as $parent_field_id => $inner_fields ){
                                        foreach( $inner_fields as $val ){
                                            ...
                                            if( strpos($key,'_confirm') !== false ){
                                                ...
                                                if( !empty( $arf_skipped_addon_fields[$key] ) ){
                                                    $final_inner_field_order[$parent_field_id][] = $arf_skipped_addon_fields[$key] .'|' . $value;
                                                } else {
                                                    $final_inner_field_order[$parent_field_id][] = $key.'|'.$value;
                                                }
                                            }
                                    ...
                                    if( !empty( $formOpt['html_running_total_field_array'] ) ){
                                        
                                        $running_total_fields_arr = $formOpt['html_running_total_field_array'];

                                        foreach( $running_total_fields_arr as $rtotal_ofkey => $rtotal_field_id ){
                                            
                                            $rtotal_fkey = $arformcontroller->arfSearchArray( $rtotal_field_id, 'old_id', $new_id_array );

                                            $rtotal_nkey = $new_id_array[ $rtotal_fkey ][ 'new_id' ];

                                            $running_total_fields_arr[ $rtotal_ofkey ] = $rtotal_nkey;


                                        }

                                        $formOpt['html_running_total_field_array'] = $running_total_fields_arr;
                                    }

                                    $newOpt = maybe_unserialize($general_option['options']);

                                    $newOpt['html_running_total_field_array'] = $formOpt['html_running_total_field_array'];
                                    ...
                                    if (!empty($new_values)) {
                                        ...
                                        $query_results = wp_cache_get('arf_import_css_'.$form_id);
                                        if( false == $query_results ){
                                            ...
                                            wp_cache_set('arf_import_css_'.$form_id, $query_results, '', 60);
                                        }
                                        ...
                                        $common_file_name = FORMPATH . '/core/css_create_common.php';
                                        $rtl_file_name = FORMPATH . '/core/css_create_rtl.php';
                                        ...
                                        if( 'standard' == $arfinputstyle || 'rounded' == $arfinputstyle ){
                                            ...
                                            include $filename;
                                            include $common_file_name;
                                            if( is_rtl() ){
                                                include $rtl_file_name;
                                            }
                                    ...
                                    if ( !empty( $_SESSION['arf_fields'] ) && is_array($_SESSION['arf_fields']) && count($_SESSION['arf_fields']) > 0 ) {
                                            ...
                                            foreach ($_SESSION['arf_fields'] as $original_id => $field_new_id) {
                                                foreach ($option_arr_new['submit_conditional_logic']['rules'] as $new_rule) {
                                                    if ($new_rule['field_id'] == $original_id) {
                                                        ...
                                                        $field_type_db = wp_cache_get('arf_import_form_field_'.$sub_cl_field_id);
                                                        if( false === $field_type_db ){
                                                            $field_type_db = $wpdb->get_row($wpdb->prepare("SELECT type FROM `".$MdlDb->fields."` WHERE id = %d",$sub_cl_field_id) );
                                                            wp_cache_set('arf_import_form_field_'.$sub_cl_field_id, $field_type_db, '', 60);
                                                        }
                                    ...
                                    if (isset( $_SESSION['arf_conditional_mail_rules'] ) && is_array($_SESSION['arf_conditional_mail_rules']) && count($_SESSION['arf_conditional_mail_rules']) > 0 ) {
                                        if (!empty($option_arr_new['arf_conditional_mail_rules'])) {
                                            foreach ($option_arr_new['arf_conditional_mail_rules'] as $new_rule) {
                                                ...
                                                $email_field_type = wp_cache_get('arf_import_form_field_'.$cl_email_field_id);
                                                if( false === $email_field_type ){
                                                    $email_field_type = $wpdb->get_row($wpdb->prepare("SELECT type FROM `".$MdlDb->fields."` WHERE id = %d",$cl_email_field_id));
                                                    wp_cache_set('arf_import_form_field_'.$cl_email_field_id, $email_field_type, '', 60);
                                                }
                                        ...
                                        foreach ($conditional_logic as $i => $value_rules) {
                                            if (isset($value_rules['condition']) && is_array($value_rules['condition'])) {
                                                foreach ($value_rules['condition'] as $j => $condition_rules) {
                                                    ...
                                                    $arf_cl_rule_field_id = isset( $_SESSION['arf_fields'][$condition_rules['field_id']] ) ? $_SESSION['arf_fields'][$condition_rules['field_id']] : '';
                                                    $arf_cl_rule_field_type = $condition_rules['field_type'];
                                                    

                                                    $cl_rule_type = wp_cache_get('arf_import_form_field_'.$arf_cl_rule_field_id);
                                                    if( false === $cl_rule_type ){
                                                        $cl_rule_type = $wpdb->get_row($wpdb->prepare("SELECT type FROM `".$MdlDb->fields."` WHERE id = %d",$arf_cl_rule_field_id));
                                                        wp_cache_set('arf_import_form_field_'.$arf_cl_rule_field_id, $cl_rule_type, '', 60);
                                                    }
                                                    ...
                                                    $conditional_logic[$i]['condition'][$j]['field_id'] = isset( $_SESSION['arf_fields'][$condition_rules['field_id']] ) ? $_SESSION['arf_fields'][$condition_rules['field_id']] : '';
                                                    if( isset( $_SESSION['arf_fields'][$condition_rules['field_id']] ) ){
                                            ...
                                            if (isset($value_rules['result']) && is_array($value_rules['result'])) {
                                                ...
                                                foreach ($value_rules['result'] as $k => $result_rules) {
                                                    ...
                                                    $arf_cl_res_field_id = isset( $_SESSION['arf_fields'][$result_rules['field_id']] ) ? $_SESSION['arf_fields'][$result_rules['field_id']] : '';
                                                    $arf_cl_res_field_type = $result_rules['field_type'];


                                                    $rs_rule_type = wp_cache_get('arf_import_form_field_'.$arf_cl_res_field_id);
                                                    if( false === $rs_rule_type ){
                                                        $rs_rule_type = $wpdb->get_row($wpdb->prepare("SELECT type FROM `".$MdlDb->fields."` WHERE id = %d",$arf_cl_res_field_id));
                                                        wp_cache_set('arf_import_form_field_'.$arf_cl_res_field_id, $rs_rule_type, '', 60);
                                                    }
                                    ...
                                    if (isset($res1[0]['responder_api_key']) && $res1[0]['responder_api_key'] != '') {
                                    
                                        foreach ($val_main->autoresponder->aweber->children() as $autores_key1 => $autores_val1) {
                                            $aweber_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                        $aweber_arr['enable'] = '';
                                    ...
                                    if (isset($res2[0]['responder_api_key']) && $res2[0]['responder_api_key'] != '') {
                                    
                                        foreach ($val_main->autoresponder->mailchimp->children() as $autores_key1 => $autores_val1) {
                                            $mailchimp_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    ...
                                    if (isset($res11[0]['responder_api_key']) && $res11[0]['responder_api_key'] != '') {
                                        
                                        foreach ($val_main->autoresponder->madmimi->children() as $autores_key1 => $autores_val1) {
                                            $madmimi_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                        $madmimi_arr['enable'] ='';
                                    ...
                                    if (isset($res3[0]['responder_api_key']) && $res3[0]['responder_api_key'] != '') {
                                         foreach ($val_main->autoresponder->getresponse->children() as $autores_key1 => $autores_val1) {
                                            $getresponse_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                    ...
                                    if (isset($res4[0]['responder_api_key']) && $res4[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->gvo->children() as $autores_key1 => $autores_val1) {
                                            $gvo_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                        $gvo_arr['enable'] = '';
                                        $gvo_arr['is_global'] = 1;
                                    ...
                                    if (isset($res5[0]['responder_api_key']) && $res5[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->ebizac->children() as $autores_key1 => $autores_val1) {
                                            $ebizac_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                        $ebizac_arr['enable'] ='';
                                    ...
                                    if (isset($res6[0]['responder_api_key']) && $res6[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->icontact->children() as $autores_key1 => $autores_val1) {
                                            $icontact_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                        $icontact_arr['enable'] ='';
                                    ...
                                    if (isset($res7[0]['responder_api_key']) && $res7[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->constant_contact->children() as $autores_key1 => $autores_val1) {
                                            $constant_contact_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                    ...
                                    if (isset($res14[0]['responder_api_key']) && $res14[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->mailerlite->children() as $autores_key1 => $autores_val1) {
                                            $mailerlite_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                    ...
                                    if (isset($res15[0]['responder_api_key']) && $res15[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->hubspot->children() as $autores_key1 => $autores_val1) {
                                            $hubspot_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                        $hubspot_arr['enable'] ='';
                                    ...
                                    if (isset($res17[0]['responder_api_key']) && $res17[0]['responder_api_key'] != '') {
                                        foreach ($val_main->autoresponder->convertkit->children() as $autores_key1 => $autores_val1) {
                                            $convertkit_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                    ...
                                    if (isset($res16[0]['responder_api_key']) && $res16[0]['responder_api_key'] != '') {
                                         foreach ($val_main->autoresponder->sendinblue->children() as $autores_key1 => $autores_val1) {
                                            $sendinblue_arr[$autores_key1] = (string) trim($autores_val1);
                                        }
                                    }else{
                                                            ...
                                                            if ($v->name == $field_nm) {
                                                                ...
                                                                if ($field_type['field_type'] == 'file' && $v->type == 'file' && trim($val_field) != '' ) {
                                                                    ...
                                                                    foreach ($old_values_field as $old_val_field) {
                                                                        ...
                                                                        $info = file_exists($image_path1 ) ? getimagesize($image_path1) : array();
                                                                        $mime_type = isset( $info['mime'] ) ? arf_sanitize_value($info['mime']) : '';
                                                                    ...
                                                                    $entry_value = array();
                                                            } elseif($field_type['field_type'] == 'matrix'){
                                                                
                                                                $values = explode(',', (string) $val_field);
                                                                $new_values = maybe_serialize($values);
                                                                $entry_values_new['item_meta'][$v->id] = $new_values;
                            ...
                            /* for import arforms lite form */
                            else if(isset($xml->arformslite)) {
                                $ik = 0;
                                foreach ($xml->children() as $formxml) {
                                    foreach ($formxml->children() as $key_main => $val_main) {
                                        ...
                                        //code end here.                                
                                        $general_option['is_importform'] = 'Yes';
                                        $general_option['is_imported_from_lite'] = 1;
                                        //code start here for store all general options in database.
                                        $autoresponder_fname = isset($general_option['autoresponder_fname']) ? $general_option['autoresponder_fname'] : 0;
                                        $autoresponder_lname = isset($general_option['autoresponder_lname']) ? $general_option['autoresponder_lname'] : 0;
                                        $autoresponder_email = isset($general_option['autoresponder_email']) ? $general_option['autoresponder_email'] : 0;
                                        ...
                                        foreach ($val_main->fields->children() as $key_fields => $val_fields) {
                                            ...
                                            foreach ($val_fields as $key_field => $val_field) {
                                                            ...
                                                            if (is_array($value)) {                                                        
                                                                if (isset($value['label_image']) && $value['label_image'] !='') {
                                                                    ...
                                                                    if( ! $arfilecontroller->arf_process_upload( $destination ) ){
                                                                        ...
                                                                        $val_field_radio[$key]['label_image'] = $destination;
                                                                    }
                                                                }
                                                            }
                                                        }
                                                ...
                                                if( $key_field == 'field_options' ){
                                                    $arf_field_options = arf_json_decode(trim($val_field),true);
                                        ...
                                        if (!empty($new_values)) {
                                            ...
                                            if( 'standard' == $arfinputstyle || 'rounded' == $arfinputstyle ){
                                                ...
                                                $css2 .= "\n " . $warn2;
                                                $css_file2 = $target_path1 . '/maincss_materialize_outlined_' . $form_id . '.css';
                                        ...
                                        /* added for conditional mail rules */
                                        $arf_conditional_mail_rules = array();
                                        if (isset($_SESSION['arf_conditional_mail_rules']) && count($_SESSION['arf_conditional_mail_rules']) > 0 and is_array($_SESSION['arf_conditional_mail_rules'])) {
                                            if (!empty($option_arr_new['arf_conditional_mail_rules'])) {
                                                foreach ($option_arr_new['arf_conditional_mail_rules'] as $new_rule) {
                                                    $_SESSION['arf_fields'][$new_rule['field_id_mail']] = isset($_SESSION['arf_fields'][$new_rule['field_id_mail']]) ? $_SESSION['arf_fields'][$new_rule['field_id_mail']] : '';
                                                    $_SESSION['arf_fields'][$new_rule['send_mail_field']] = isset($_SESSION['arf_fields'][$new_rule['send_mail_field']]) ? $_SESSION['arf_fields'][$new_rule['send_mail_field']] : '';
                                        ...
                                        $conditional_logic = isset($option_arr_new['arf_conditional_logic_rules']) ? $option_arr_new['arf_conditional_logic_rules'] : array();
                                        /* remove conditional logic condition because conditional logic doesnt set in arformslite */
                                        $option_arr_new = maybe_serialize(apply_filters('arf_import_update_field_outside', $option_arr_new, $_SESSION['arf_fields'], $form_id));
                                        ...
                                        if (isset($res1[0]['responder_api_key']) && $res1[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->aweber )) {
                                            foreach ($val_main->autoresponder->aweber->children() as $autores_key1 => $autores_val1) {
                                                $aweber_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res2[0]['responder_api_key']) && $res2[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->mailchimp )) {
                                        ...
                                        $mailchimp = maybe_serialize($mailchimp_arr);
                                        if (isset($res11[0]['responder_api_key']) && $res11[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->madmimi ) ) {
                                        ...
                                        if (isset($res3[0]['responder_api_key']) && $res3[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->getresponse )) {
                                             foreach ($val_main->autoresponder->getresponse->children() as $autores_key1 => $autores_val1) {
                                                $getresponse_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res4[0]['responder_api_key']) && $res4[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->gvo )) {
                                            foreach ($val_main->autoresponder->gvo->children() as $autores_key1 => $autores_val1) {
                                                $gvo_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res5[0]['responder_api_key']) && $res5[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->ebizac )) {
                                            foreach ($val_main->autoresponder->ebizac->children() as $autores_key1 => $autores_val1) {
                                                $ebizac_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res6[0]['responder_api_key']) && $res6[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->icontact ) ) {
                                            foreach ($val_main->autoresponder->icontact->children() as $autores_key1 => $autores_val1) {
                                                $icontact_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if ( isset($res7[0]['responder_api_key']) && $res7[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->constant_contact ) ) {
                                            foreach ($val_main->autoresponder->constant_contact->children() as $autores_key1 => $autores_val1) {
                                                $constant_contact_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        } else {
                                        ...
                                        if (isset($res14[0]['responder_api_key']) && $res14[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->mailerlite ) ) {
                                            foreach ( $val_main->autoresponder->mailerlite->children() as $autores_key1 => $autores_val1) {
                                                $mailerlite_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res15[0]['responder_api_key']) && $res15[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->hubspot ) ) {
                                            foreach ($val_main->autoresponder->hubspot->children() as $autores_key1 => $autores_val1) {
                                                $hubspot_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res17[0]['responder_api_key']) && $res17[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->convertkit ) ) {
                                            foreach ($val_main->autoresponder->convertkit->children() as $autores_key1 => $autores_val1) {
                                                $convertkit_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if (isset($res16[0]['responder_api_key']) && $res16[0]['responder_api_key'] != '' && isset( $val_main->autoresponder->sendinblue )) {
                                             foreach ($val_main->autoresponder->sendinblue->children() as $autores_key1 => $autores_val1) {
                                                $sendinblue_arr[$autores_key1] = (string) trim($autores_val1);
                                            }
                                        }else{
                                        ...
                                        if( !empty( $res18[0]['responder_api_key'] ) && isset( $val_main->autoresponder->drip ) ){
                                            foreach( $val_main->autoresponder->drip->children() as $autores_key1 => $autores_val1 ){
                                                $drip_arr[$autores_key1] = (string) trim( $autores_val1 );
                                            }
                                        } else {
                                            $drip_arr['enable'] = '';
                                        ...
                                        $update = $wpdb->query($wpdb->prepare("insert into " . $MdlDb->ar . " (aweber ,mailchimp, getresponse, gvo, ebizac,madmimi , icontact, constant_contact,mailerlite,sendinblue , hubspot, convertkit, drip, enable_ar,  frm_id) values ('%s', '%s', '%s', '%s', '%s','%s', '%s', %s, '%s', '%s', '%s', '%s', '%s', '%s', '%d')", $aweber, $mailchimp, $getresponse, $gvo, $ebizac, $madmimi, $icontact, $constant_contact, $mailerlite,$sendinblue, $hubspot, $convertkit, $drip, maybe_serialize(json_decode(trim($val_main->autoresponder->enable_ar),true)), $frm_id));
                                    ...
                                    <div class="message_descripiton">
                                        ...
                                        <div class="message_svg_icon">
                                            <svg style="height: 14px;width: 14px;">
                                                <path fill-rule="evenodd" clip-rule="evenodd" fill="#FFFFFF" d="M6.075,14.407l-5.852-5.84l1.616-1.613l4.394,4.385L17.181,0.411l1.616,1.613L6.392,14.407H6.075z"></path>
                                            </svg>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="modal-body" style="clear:both;padding:15px;">
                        ...
                        <div class="opt_export_div">
                            ...
                            <span class="arf_helptip_container">
                                <a href="<?php echo ARFURL; ?>/documentation/index.html#import_export" target="_blank" title="" class="arf_adminhelp_icon">
                        ...
                        <div class="import_opt_part" id="import_opt_part" style="display:block;margin-top:-9px;">
                            <form  action="" method="post" enctype="multipart/form-data" >
                                <table class="form-table">
                                    ...
                                    <tr>
                                        <td style="padding:0;">
                                            <input type="hidden" name="arf_xml_file_name" id="arf_xml_file_name" value="" /><input type="hidden" name="arf_import_disable" id="arf_import_disable" value="1" /><input type="hidden" name="arf_nonce" value="<?php echo wp_create_nonce( 'arf_import_form' ); ?>" />
