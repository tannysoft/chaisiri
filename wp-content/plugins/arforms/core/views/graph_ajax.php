...
if ($type == "daily") {
    ...
    for ($z = 0; $z <= 23; $z+=2) {
        if ((isset($new_arr[$z]) and $new_arr[$z] != "") || (isset($new_arr[$z + 1]) and $new_arr[$z + 1] != "")) {
            $new_array[$z] = ( isset($new_arr[$z]) ? $new_arr[$z] : 0 ) + ( isset($new_arr[$z + 1]) ? $new_arr[$z + 1] : 0 );
        }
        if ((isset($newViewArr[$z]) and $newViewArr[$z] != "") || (isset($newViewArr[$z + 1]) and $newViewArr[$z + 1] != "")) {
            $newViewArray[$z] = ( isset($newViewArr[$z]) ? $newViewArr[$z] : 0 ) + ( isset($newViewArr[$z + 1]) ? $newViewArr[$z + 1] : 0 );
        }
    }
    $dailyline1 = '';
    $dailyline2 = '';
