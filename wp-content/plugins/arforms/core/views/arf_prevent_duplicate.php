...
class arf_prevent_duplicate {
    ...
    function __construct() {
        ...
        add_filter('arf_additional_form_content_outside', array($this, 'arf_prevent_duplicate_after_form_render'), 10,5);
    ...
    function arf_prevent_duplicate_data_onsubmit($id, $values) {
        ...
        global $armainhelper, $arformcontroller,$arfieldhelper, $maincontroller;
        ?>
                ...
                <div class="sltstandard">
                    ...
                    if (!empty($values['fields'])) {
                        foreach ($values['fields'] as $field2) {
                            if (in_array($field2['type'], array('email', 'text', 'hidden', 'radio', 'select', 'phone')) && $field2['type'] != 'arf_repeater') {

                                if( isset( $field2['parent_field_type'] ) && $field2['parent_field_type'] == 'arf_repeater' ){
                                    continue;
                                }
                                ...
                                if($current_field_id !="" && $arfieldhelper->arf_execute_function($field2["name"],'strip_tags')=="" ){
                                    ...
                                    $selectbox_field_options[$current_field_id] = $arfieldhelper->arf_execute_function($field2["name"],'strip_tags');
                                }
                    ...
                    echo $maincontroller->arf_selectpicker_dom( 'options[arf_pre_dup_field]', 'arf_pre_dup_field', 'arf_pre_dup_field_dropdown', 'width:240px;', $user_responder_email, array(), $selectbox_field_options, false, array(), false, array(), false, array(), true );
                    ?>  
    ...
    function arf_prevent_duplicate_before_form_render($arf_form,$form) {
        if( !isset( $form->form_css['arfsuccessmsgposition'] ) || ( isset($form->form_css['arfsuccessmsgposition']) && $form->form_css['arfsuccessmsgposition'] != 'bottom' ) ){
            global $wpdb,$MdlDb;

            $form_id2 = $form->id;

                $form_data2 = wp_cache_get('arf_form_data_'.$form_id2);
                if( false == $form_data2){
                    $form_data2 = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$MdlDb->forms.' WHERE id = %d', $form_id2), 'ARRAY_A');                    
                    wp_cache_set('arf_form_data_'.$form_id2, $form_data2);
                }
    ...
    function arf_prevent_duplicate_after_form_render($arf_form, $form, $arf_data_uniq_id, $arfbrowser_name, $browser_info) {
