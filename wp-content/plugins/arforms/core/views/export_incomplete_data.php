...
global $arrecordhelper,$arrecordcontroller,$maincontroller,$arfieldhelper,$armainhelper,$arfsettings,$arfrecordmeta,$MdlDb;
	...
	$entries = $db_record->getAll($where_clause, '', '', true, false,'',array(),true);
	
	$form_cols	= apply_filters('arfpredisplayformcolsincompleteentries', $form_cols, $form->id);
	$entries	= apply_filters('arfpredisplaycolsitemsincompleteenteries', $entries, $form->id);
	...
	echo '"'. addslashes(esc_html__('Timestamp', 'ARForms')) .'"'.$entry_separator.'"IP"'.$entry_separator.'"Country"'.$entry_separator.'"Browser"'.$entry_separator.'"Page URL"'.$entry_separator.'"Referrer URL"'."\n";

	foreach($entries as $entry){
		global $wpdb,$MdlDb;
		echo "\"{$entry->id}\"$entry_separator";
		$res_data = $wpdb->get_results( $wpdb->prepare('SELECT description,country, browser_info FROM '.$MdlDb->incomplete_entries.' WHERE id = %d', $entry->id), 'ARRAY_A');
