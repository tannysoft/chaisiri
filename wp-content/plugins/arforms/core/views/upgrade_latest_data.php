...
if (version_compare($newdbversion, '1.0', '>') || version_compare($newdbversion, '1', '=')) {
    ...
    if (version_compare($newdbversion, '4.0', '<')) {
        require FORMPATH . '/core/views/upgrade_latest_data_v4.0.php';
    }

    if( version_compare( $newdbversion, '4.1', '<' ) ){
        $wpdb->query( "ALTER TABLE `" . $MdlDb->forms . "` ADD `is_imported_from_lite` TINYINT(1) NOT NULL DEFAULT '0'" );
    }

    if( version_compare( $newdbversion, '4.2', '<') ){
        $wpdb->query( "ALTER TABLE `" . $MdlDb->forms . "` ADD `partial_grid_column_list` TEXT NULL AFTER `is_imported_from_lite`");
    }

    if( version_compare( $newdbversion, '4.3', '<' ) ){
        
        $wpdb->query("ALTER TABLE `" . $MdlDb->forms . "` ADD `arf_is_lite_form` TINYINT(1) DEFAULT 0");

        $wpdb->query("ALTER TABLE `" . $MdlDb->forms . "` ADD `arf_lite_form_id` INT(11) DEFAULT NULL");

        $wpdb->query("ALTER TABLE `" . $MdlDb->ar . "`  ADD `drip` TEXT NOT NULL");

        $wpdb->query("INSERT INTO `" . $MdlDb->autoresponder. "` (responder_id) VALUES (".arf_sanitize_value(18, 'integer').")");
        ...
        $updateoptionsetting->arfdisplayfilesinwpmedia = true;
        ...
        foreach( $res as $key => $val ){
            ...
            $arform_fields = $wpdb->get_results( $wpdb->prepare( "SELECT `id`, `field_options` FROM `" . $MdlDb->fields ."` WHERE form_id = %d", $arform_id ), OBJECT_K );
            foreach( $arform_fields as $fk => $f_val ){
                if( isset( $f_val->field_options ) ){

                    $field_options = arf_json_decode($f_val->field_options,true);
    ...
    if( version_compare( $newdbversion, '4.3.3', '<' ) ){

        $autoresponder_table = $MdlDb->autoresponder;

        $getAweberData = $wpdb->get_row( $wpdb->prepare( "SELECT responder_list_id, is_verify FROM `" . $autoresponder_table . "` WHERE is_verify = %d AND responder_id = %d", 1, 3 ) );

        if( !empty( $getAweberData ) ){
            ...
            update_option( 'arf_reauthorize_aweber', true );
	...
	if( version_compare( $newdbversion, '4.4', '<') ){
		global $arformcontroller;
        $is_updated_entries = $arformcontroller->arf_update_form_entries();
    }

    if( version_compare( $newdbversion, '5.0', '<') ){
        require FORMPATH . '/core/views/upgrade_latest_data_v5.0.php';
    }

    if( version_compare( $newdbversion, '5.6', '<') ){
        ...
        update_option( 'arf_5_6_update_date', date( 'Y-m-d H:i:s' ) );
        ...
        $wpdb->query("ALTER TABLE `".$MdlDb->forms."` ADD `arf_update_form` TINYINT(1) NOT NULL DEFAULT '0'");

        $get_form_ids = $wpdb->get_results( $wpdb->prepare( "SELECT form_id FROM `" . $MdlDb->fields . "` WHERE form_id > %d AND ( type = %s OR type = %s OR type = %s OR type = %s OR type = %s ) GROUP BY form_id", 11, 'select', 'slider', 'arf_multiselect', 'checkbox', 'radio' ) );
    ...
    if( version_compare( $newdbversion, '5.7', '<') ){
        global $wpdb, $db_record, $MdlDb, $armainhelper, $arfieldhelper, $arsettingcontroller, $arfsettings, $arffield, $arfrecordmeta, $arfform, $style_settings, $maincontroller;
        ...
        /** UPDATE ALL FORMS EXCEPT TEMPLATES AND SET FLAG TO REWRITE THE CSS */
        $wpdb->update(
            $MdlDb->forms,
            array(
                'arf_update_form' => 1
    ...
    update_option('arf_db_version', '5.7.1');

    global $newdbversion;
    $newdbversion = '5.7.1';

    update_option('arf_new_version_installed', arf_sanitize_value(1, 'integer'));
    update_option('arforms_updated_date_' . $newdbversion, current_time( 'mysql' ) );
}
?>