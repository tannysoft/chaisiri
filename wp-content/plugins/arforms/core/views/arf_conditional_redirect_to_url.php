...
class ARF_conditional_redirect_to_url {
                ...
                <div class="arfcolmnleft">
                    <div class="arftablerow" style="margin-left:0;">
						...
						global $conditional_logic_array_if;
                                                ...
                                                $selectbox_field_opts_attr = array();
                                                ...
                                                if (!empty($values['fields'])) {
                                                    foreach ($values['fields'] as $val_key => $fo) {
                                                        if (!in_array($fo['type'], $conditional_logic_array_if)) {
                                                            ...
                                                            $selectbox_field_opts_attr['data-type'][$current_field_id] = $fo["type"];
                                                ...
                                                <?php
                                                    ...
                                                    $arf_conditional_redirect_attr = array(
                                                        'class' => 'arf_conditional_redirect_field_action'
                                                    );

                                                    echo $maincontroller->arf_selectpicker_dom( 'options[arf_conditional_redirect_rules]['. $rule_i .'][field_id]', 'arf_conditional_redirect_filed_'.$rule_i, 'arf_name_field_dropdown', 'width:100%;', $redirect_value['field_id'], $arf_conditional_redirect_attr, $selectbox_field_options, false, array(), false, $selectbox_field_opts_attr, false, array(), true );
                                                ?>
                                            </div>
                                        </span>
                                        ...
                                        <span id="select_ar_conditional_redirect_operator_url">
                                            <div class="sltstandard" style="width: 100%;">
                                                <?php echo $arfieldhelper->arf_cl_rule_menu_for_conditional_redirect('arf_conditional_redirect_filed_operator_' . $rule_i, 'arf_conditional_redirect_filed_operator_' . $rule_i, $redirect_value['operator'],$rule_i,$redirect_value['field_type']); ?>
                                            </div>
                                        </span>

                                        <span id="select_ar_conditional_redirect_value_url" style="">
                                            <input style="width:100%" type="text" class="txtstandardnew arfheight34" placeholder="<?php echo (esc_attr($redirect_value['field_type']) == "date") ? 'YYYY/MM/DD':'' ;?>" value="<?php echo $redirect_value['value']; ?>" id="arf_conditional_redirect_filed_value_<?php echo $rule_i; ?>" onkeyup="this.setAttribute('value',this.value)" name="options[arf_conditional_redirect_rules][<?php echo $rule_i; ?>][value]" /> 
                                        </span>
        ...
        <script type="text/javascript">
            ...
            function arf_conditional_redirect_enable_disable() {
                if (jQuery("#arf_conditional_redirect_enable").is(':checked'))
                {
                    ...
                    jQuery('#success_url').prop('disabled', true);

                } else {
                    jQuery("#arf_rule_conditional_redirect_mian").hide();
                    jQuery('#success_url').prop('disabled', false);
                }
            }
        </script>    
        <?php
            ...
            <div style="float: left; width: 100%">
                ...
                <span id="select_ar_conditional_redirect_filed" style="width:170px; float: left;">
                    <div class="sltstandard">
                        <?php
                            $arf_cond_redirect_url_opts = array( '' => addslashes(esc_html__('Select Field', 'ARForms')));
                            echo $maincontroller->arf_selectpicker_dom( 'arf_conditional_redirect_filed_'.$rule_i, 'arf_conditional_redirect_filed_'.$rule_i, '', 'width:160px;', '', array(), $arf_cond_redirect_url_opts, false, array(), false, array(), false, array(), true );
                        ?>
                    </div>
                </span>
    ...
    function arf_importtime_update_conditional_redirect_field_id($option_arr_new, $old_new_field_id, $form_id) {
        global $wpdb,$MdlDb;
        if ( isset($option_arr_new['arf_conditional_redirect_rules']) && count($option_arr_new['arf_conditional_redirect_rules']) > 0 and is_array($option_arr_new['arf_conditional_redirect_rules'])) {
            $arf_conditional_redirect_rules = array();
            if (!empty($option_arr_new['arf_conditional_redirect_rules'])) {
                foreach ($option_arr_new['arf_conditional_redirect_rules'] as $new_rule) {
                    if( empty( $new_rule['field_id'] ) ){
                        continue;
                    }
