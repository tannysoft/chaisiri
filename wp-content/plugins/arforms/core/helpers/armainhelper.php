...
class armainhelper {
    ...
    function wp_pages_dropdown($field_name, $page_id, $truncate = false, $id = '') {
        ...
        global $wpdb, $armainhelper,$maincontroller;
        ...
        foreach ($pages as $page) {
            ...
            if ((isset($_POST[$field_name]) and $_POST[$field_name] == $page->ID) or ( !isset($_POST[$field_name]) and $page_id == $page->ID) || $cntr == 0) {
                ...
                $arf_cl_field_selected_option['name'] = !empty( $post_title_value ) ? $post_title_value : esc_html__('Page ID:', 'ARForms') .' '. $page->ID;
            }

            $arf_cl_field_options[$page->ID] = !empty( $post_title_value ) ? $post_title_value : esc_html__('Page ID:', 'ARForms') .' '. $page->ID;
            $cntr++;
        }
        ...
        $margin_text = 'margin-left';
        if( is_rtl() ){
            $margin_text = 'margin-right';
        }
        echo $maincontroller->arf_selectpicker_dom( $field_name, $selec_id. '_arf_wp_pages', '', 'width:240px; '.$margin_text.': 10px; clear: none;', $arf_cl_selected_page_id, array(), $arf_cl_field_options, false, array(), false, array(), false, array(), true );
    }
    ...
    function get_countries() {
        ...
        return apply_filters('arfcountries', array(
            ...
            addslashes(esc_html__('Lithuania', 'ARForms')), addslashes(esc_html__('Luxembourg', 'ARForms')), addslashes(esc_html__('North Macedonia', 'ARForms')),
    ...
    function upload_file($field_id, $fomr_id = null) {
        ...
        $media_id = media_handle_upload_custom_v4($field_id, 0, $fomr_id);
    ...
    function frm_capabilities() {
        ...
        $cap = array(
            ...
            'arfchangesettings' => addslashes(esc_html__('Access the Settings Page', 'ARForms')),
            'arfimportexport' => addslashes(esc_html__('Access the Import/Export Page', 'ARForms')),
        ...
        $cap['arfviewincompleteentries'] = addslashes( esc_html__('View ARForms incomplete form data', 'ARForms') );

        $cap['arfeditincompleteentries'] = addslashes( esc_html__('Edit ARForms incomplete form data', 'ARForms') );

        $cap['arfdeleteincompleteentries'] = addslashes( esc_html__('Delete ARForms incomplete form data', 'ARForms') );
    ...
    function arf_update_fa_font_class($value){
        ...
        $value = str_replace('arfar arfa', 'far fa', $value);
        $value = str_replace('arfab arfa', 'fab fa', $value);
        $value = str_replace('arfas arfa', 'fas fa', $value);
        $value = str_replace('arfa arfa', 'fas fa', $value);
