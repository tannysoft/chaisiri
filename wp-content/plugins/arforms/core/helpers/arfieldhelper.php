...
class arfieldhelper {
    ...
    function __construct() {
        ...
        add_filter('arf_check_for_running_total_field_arr', array($this, 'arf_check_running_total_field_arr_func'), 10, 5);

        add_action('arf_material_style_editor_content', array($this, 'arf_add_material_style_block'), 10, 5);

        add_action('arf_material_outlined_style_editor_content', array( $this, 'arf_add_material_outlined_style_block'), 10, 5 );

        add_filter('arf_form_fields_outside',array($this, 'arf_add_blank_msg_from_globalsetting'),100,2);

    }

    function arf_input_field_keys(){
        ...
        $inputFields = apply_filters( 'arform_input_fields', $inputFields );

        return $inputFields;

    }

    function arf_other_fields_keys(){
        ...
        $otherFields = apply_filters( 'arform_other_fields', $otherFields );
    ...
    function arf_field_element_orders(){
        $fields = array(
            'text',
            'textarea',
            'checkbox',
        ...
        return apply_filters( 'arf_manage_field_element_order_outside', $fields );
    }

    function arf_full_width_field_element(){
        $field_elm = array(
            'break'
        );

        return apply_filters( 'arf_manage_full_width_field_element_outside', $field_elm );
    }
    ...
    function setup_new_variables($type = '', $form_id = '') {
        ...
        else if ($type == 'arf_multiselect')
            $values['options'] = maybe_serialize(array('', addslashes(esc_html__('Select 1', 'ARForms')), addslashes(esc_html__('Select 2', 'ARForms'))));
    ...
    function get_default_field_options($type, $field, $limit = false) {
        ...
        $field_options = array(
            ...
            'first_page_label' => 'Step1', 'second_page_label' => 'Step2', 'pre_page_title' => addslashes(esc_html__('Previous', 'ARForms')), 'next_page_title' => addslashes(esc_html__('Next', 'ARForms')), 'page_break_type' => 'wizard', 'page_break_first_use' => '0', 'pagebreaktabsbar' => 0, 'page_break_type_possition' => 'top', 'pagebreakclickable' => 0, 'is_recaptcha' => 'recaptcha',
            'inline_css' => '', 'css_outer_wrapper' => '', 'css_label' => '', 'css_input_element' => '', 'css_description' => '',
            'file_upload_text' => addslashes(esc_html__('Upload', 'ARForms')), 'max_fileuploading_size' => 'auto', 'upload_btn_color' => '#077bdd', 'arf_divider_font' => 'Helvetica',
            'arf_divider_font_size' => '16', 'arf_divider_font_style' => 'bold', 'arf_divider_bg_color' => '#ffffff', 'arf_divider_inherit_bg' => '0', 'arf_section_font' => 'Helvetica',
            'arf_section_font_size' => '16', 'arf_section_font_style' => 'bold', 'arf_section_bg_color' => '#ffffff', 'arf_section_inherit_bg' => '0',
        ...
        if ($type == 'captcha') {
            ...
            $field_options['min_error_msg'] = addslashes( esc_html__( 'The Number is below range', 'ARForms' ) );
            $field_options['max_error_msg'] = addslashes( esc_html__( 'The number is above range', 'ARForms' ) );
        } else if ($type == 'phone') {
    ...
    function replace_field_shortcodes($html, $field, $errors = array(), $fields = array() , $form = false) {
        ...
        foreach (array('description' => $field['description'], 'required_label' => $required, 'error' => $error) as $code => $value) {
            ...
            if ($code == 'description') {
                if ($field['type'] != 'html' && $field['type'] != 'divider' && $field['type'] != 'section')
                    $value = '';
            }
        ...
        $html = str_replace('[label_position]', (($field['type'] == 'divider' or $field['type'] == 'section' or $field['type'] == 'break') ? $field['label'] : ' arf_main_label'), $html);
    ...
    function get_file_icon($media_id) {
        ...
        $image_array_link = isset( $post_meta_data["_wp_attached_file"] ) ? $post_meta_data["_wp_attached_file"][0] : '';

        if( empty( $image_array_link ) ){
            return;
        }
        ...
        if( in_array($image_ext,$exclude_ext) ){
            ...
            $img_height = '';
            $img_width = '';
            $image_array_link = wp_mime_type_icon( $media_id );
        }
    ...
    function get_file_name_link($media_id, $short = true) {
        if (is_numeric($media_id)) {
            ...
            $url = isset( $url["_wp_attached_file"] ) ? $url["_wp_attached_file"][0] : '';
    ...
    function get_field_options($form_id, $value = '', $include = 'not', $types = "'break','divider','section','data','file','captcha'", $data_children = false) {
    ...
    function get_display_value($replace_with, $field, $atts = array(), $is_for_mail = false) {
        ...
        if ($field->type == 'date') {
            ...
            $replace_with = $this->get_date_entry($replace_with, $field->form_id, $fieldopt['show_time_calendar'], $fieldopt['clock'], $fieldopt['locale']);
    ...
    function arf_cl_field_menu($form_id, $select_name, $select_id = '', $default_field_id = 0) {
        global $arffield, $arfieldhelper,$maincontroller;
        ...
        $fields = $arffield->getAll("fi.type not in ('divider', 'section', 'captcha', 'break', 'html', 'file', 'imagecontrol') and fi.form_id=" . (int) $form_id, 'id');
        ...
        if (count($fields) > 0) {
            ...
            foreach ($fields as $field) {
                ...
                if($field_id !="" && $this->arf_execute_function($field->name,'strip_tags') ==""){
                    $arf_cl_field_options[$field_id] = '[Field Id:'.$field_id.']';
                }else{
                    $arf_cl_field_options[$field_id] = $this->arf_execute_function($field->name,'strip_tags');
                }
                
                $cntr++;
            }

            echo $maincontroller->arf_selectpicker_dom( $select_name, $select_id, '', 'width:130px;', $arf_cl_field_selected_option['field_id'], array(), $arf_cl_field_options, false, array($slider_class), false, array(), false, array(), true );
        }
    }
    ...
    function arf_cl_field_menu_submit_cl($form_id, $select_name, $select_id = '', $default_field_id = 0) {
        ...
        $fields = $arffield->getAll("fi.type not in ('divider', 'section', 'captcha', 'break', 'html', 'file', 'imagecontrol','colorpicker','arf_product','arf_signature','arf_repeater', 'time') and fi.form_id=" . (int) $form_id, 'id');

        if (count($fields) > 0) {

            $select_id = (isset($select_id)) ? $select_id : $select_name;

            $arf_cl_field_options = array('' => addslashes(esc_html__('Select Field','ARForms')));
            $cntr = 0;
            foreach ($fields as $field) {
                ...
                if( isset( $field->field_options['parent_field'] ) ){
                    continue;
                }
                $slider_class = '';
                if ($field->type == 'arfslider') {
            ...
            echo $maincontroller->arf_selectpicker_dom( $select_name, $select_id, 'arf_condition_field_dropdown', 'width:130px;', $arf_cl_field_selected_option['field_id'], array(), $arf_cl_field_options, false, array( $slider_class ), false, array(), false, array(), true );
        }
    }

    function arf_cl_condition_operator_menu($select_name, $select_id = '', $default_rule = 'is', $ri = 0, $ci = 0, $field_type = '') {
        ...
        foreach ($conditional_rules as $rule_id => $rule) {
            if ( isset( $field_type ) && ('date' == $field_type || 'arf_spinner' == $field_type) ) {
                if ( 'contains' == $rule_id || 'not contains' == $rule_id ) {
                    $date_class[$rule_id] = 'arfhidden';
                }else{
                    $date_class[$rule_id] = '';
                }
            } else if( isset( $field_type ) && ('url' == $field_type || "website" == $field_type) ){
                if ( 'greater than' == $rule_id || 'less than' == $rule_id ) {
                    $date_class[$rule_id] = 'arfhidden';
                }else{
                    $date_class[$rule_id] = '';
                }
            ...
            if($rule_id !="" && $this->arf_execute_function($rule,'strip_tags')==""){
                $arf_cl_field_options[$rule_id] = '[Field Id:'.$rule_id.']';
            }else{
                $arf_cl_field_options[$rule_id] = $this->arf_execute_function($rule,'strip_tags');
            }
            $cntr++;
        }


        echo $maincontroller->arf_selectpicker_dom( 'options[arf_conditional_logic_rules]['.$ri.'][condition]['.$ci.'][operator]', $select_id, '', '', $arf_cl_field_selected_option['rule_id'], array(), $arf_cl_field_options, false, $date_class, false, array(), false, array(), true );
    }

    function arf_cl_rule_for_conditional_email($select_name, $select_id, $default_rule = 'is', $rule_i = 1, $field_type = '' ){
        global $maincontroller;
        $conditional_rules = array(
            'is' => addslashes(esc_html__('equals', 'ARForms')),
            'is not' => addslashes(esc_html__('not equals', 'ARForms')),
        ...
        echo $maincontroller->arf_selectpicker_dom( 'options[arf_conditional_mail_rules]['.$rule_i.'][operator_mail]', $select_id, '', 'width:130px;', $arf_cl_field_selected_option['rule_id'], array(), $arf_cl_field_options, false, $date_class, false, array(), false, array(), true );
    }

    function arf_cl_rule_menu_for_conditional_redirect($select_name, $select_id, $default_rule,$rule_i,$field_type = ''){
        ...
        echo $maincontroller->arf_selectpicker_dom( 'options[arf_conditional_redirect_rules]['.$rule_i.'][operator]', $select_id, '', '', $arf_cl_field_selected_option['rule_id'], array(), $arf_cl_field_options, false, $date_class, false, array(), false, array(), true );
    }

    function arf_cl_rule_menu_for_conditional_subscription($select_name, $select_id, $default_rule,$rule_i,$field_type = ''){
        ...
        echo $maincontroller->arf_selectpicker_dom( 'options[arf_condition_on_subscription_rules]['.$rule_i.'][operator]', $select_id, '', '', $arf_cl_field_selected_option['rule_id'], array(), $arf_cl_field_options, false, $date_class, false, array(), false, array(), true );
    }

    function arf_cl_rule_menu($select_name, $select_id = '', $default_rule = 'is', $field_type = '') {
        ...
        foreach ($conditional_rules as $rule_id => $rule) {
            ...
            if (( isset($default_rule) && $default_rule == $rule_id ) || ( $cntr == 0 )) {
                $arf_cl_field_selected_option['rule_id'] = $rule_id;
                $arf_cl_field_selected_option['rule'] = $rule;
            }
            if($rule_id !="" && $this->arf_execute_function($rule,'strip_tags')==""){
        ...
        echo $maincontroller->arf_selectpicker_dom( $select_name, $select_id, '', '', $arf_cl_field_selected_option['rule_id'], array(), $arf_cl_field_options, false, $date_class, false, array(), false, array(), true );
    }
    ...
    function arf_check_running_total_field_arr_func( $arf_on_change_function_arr = array(), $field = '', $arf_data_uniq_id = '', $form = '', $res_data = array() ){
        
        global $arformcontroller, $wpdb, $MdlDb;
        if( empty($field) || $field == '' || is_admin() ){
            return $arf_on_change_function_arr;
        }
        ...
        if( isset($field['enable_running_total']) && $field['enable_running_total'] != '' && $field['enable_running_total'] > 0 ){
            ...
            foreach($field_running_total as $ki => $frt ){
                ...
                if( (string)$key == '' ){
                    ...
                    $html_field = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . $MdlDb->fields . " WHERE form_id = %d AND id = %d ORDER BY id", $form->id, $frt));
            ...
            if( !empty( $arf_on_change_function_arr ) ){

            } else {
                $onKeyFields = apply_filters( 'arf_onchange_only_click_event_outside', array('checkbox', 'radio', 'scale', 'select', 'arfslider', 'arf_multiselect' ) );
                if( !empty( $field['field_key'] ) ){
                    if( in_array( $field['type'], $onKeyFields ) ){
                        $arf_on_change_function_arr['onchange'] = "clearTimeout(__arf_timeout_handle); __arf_timeout_handle = setTimeout(function(){arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}]);},100);";
                    } else {
                        $arf_on_change_function_arr['onkeyup'] = "clearTimeout(__arf_timeout_handle); __arf_timeout_handle = setTimeout(function(){arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}])},100);";

                    }
                    $arf_on_change_function_arr['data-arf-keyup'] = "arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}]);";
                }
            }
    ...
    function arf_check_running_total_field_func($arf_on_change_function = '',$field = '',$arf_data_uniq_id = '',$form = '',$res_data = array() ){
        ...
        if( isset($field['enable_running_total']) && $field['enable_running_total'] != '' && $field['enable_running_total'] > 0 ){
            ...
            if( $arf_on_change_function != '' ){
                ...
                if (!empty($field['field_key'])) {
                    if (in_array($field['type'], $onKeyFields)) {
                        $arf_on_change_function .= " onChange='clearTimeout(__arf_timeout_handle); __arf_timeout_handle = setTimeout(function(){arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}]);},100);' data-arf-keyup='arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}]);'";
                    } else {
                        $arf_on_change_function .= " onkeyup='clearTimeout(__arf_timeout_handle); __arf_timeout_handle = setTimeout(function(){arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}])},100);' data-arf-keyup='arf_calculate_total(\"".$form->form_key."\",\"{$field['id']}\",[{$field_running_total}]);' ";
                    }
                }
            }
        }
    ...
    function arf_field_on_change_function( $field_id = '', $unique_id = '', $form_options = '', $field_type = '', $return_as_array = false) {
        ...
        $return = "";
        $onKeyFields = apply_filters('arf_onchange_only_click_event_outside', array('checkbox', 'radio', 'scale','select','arfslider','date', 'arf_multiselect','html'));
        if (!empty($field_keys)) {
            if (in_array($field_type, $onKeyFields)) {
                $return .= " onChange='arf_cl_apply_v3(" . $unique_id . "," . json_encode($field_keys) . ");'";

                $return_arr['onchange'] = "arf_cl_apply_v3(" . $unique_id . "," . json_encode($field_keys) . ");";
            } else {
                ...
                $return_arr['onkeyup'] = "clearTimeout(__arf_timeout_handle); __arf_timeout_handle = setTimeout(function(){arf_cl_apply_v3(" . $unique_id . "," . json_encode($field_keys) . ");},300);";
            }
        }
        if( $return_as_array ){
            return $return_arr;
    ...
    function calculate_rule($field, $operator, $value, $submit = false) {
        ...
        if (isset($field['type'])) {
            ...
            if ($field['type'] == 'radio' || $field['type'] == 'select' || $field['type'] == ARF_AUTOCOMPLETE_SLUG || $field['type'] == 'arf_multiselect') {
        ...
        if (!empty($field_list)) {
            ...
            foreach ($field_list as $field) {
                ...
                if ($type == 'email' && $target_id != 'options_admin_reply_to_notification' && $target_id != 'ar_admin_from_email' && $target_id != 'ar_user_from_email' && $target_id != 'admin_email_subject' && $target_id != 'options_ar_admin_from_name' && $target_id != 'options_admin_cc_email_notification' && $target_id != 'options_admin_bcc_email_notification') {
                    if ( (!isset($field->field_options['parent_field_type']) || (isset($field->field_options['parent_field_type']) && $field->field_options['parent_field_type'] != 'arf_repeater')) && ($field->type == 'email' || $field->type == 'text' || $field->type == 'radio' || $field->type == 'select' ) ) {
                        ?>
                        <?php 
                    ...
                    if ( (!isset($field->field_options['parent_field_type']) || (isset($field->field_options['parent_field_type']) && $field->field_options['parent_field_type'] != 'arf_repeater')) && ($field->type == 'email' || $field->type == 'text' || $field->type == 'radio' || $field->type == 'select' || $field->type == 'hidden') ) {
                        ?>
                         <?php 
                        $field_name = $armainhelper->truncate($arfieldhelper->arf_execute_function($field->name,'strip_tags'), 60);
                        if($field->id !="" && $field_name=="" ){
                    ...
                    if ($is_total_field) {
                        if ($field->type != 'html' && (!isset($field->field_options['parent_field_type']) || (isset($field->field_options['parent_field_type']) && $field->field_options['parent_field_type'] != 'arf_repeater'))) {
                            ?>
                           <?php 
                            $field_name = $armainhelper->truncate($arfieldhelper->arf_execute_function($field->name,'strip_tags'), 60);
                            if($field->id !="" && $field_name=="" ){
                        ...
                        if ( (!isset($field->field_options['parent_field_type']) || (isset($field->field_options['parent_field_type']) && $field->field_options['parent_field_type'] != 'arf_repeater')) && ( $target_id == "ar_email_subject" || $target_id == 'admin_email_subject' ) && $field->type != 'html' && $field->type != 'file' && $field->type != 'like' && $field->type != 'arf_repeater' && $field->type != 'matrix' ) {
                            ?>
                         ...
                         <?php 
                            ...
                            <?php
                        } else if ( ( $target_id != "ar_email_subject" && $target_id != 'admin_email_subject' ) && (!isset($field->field_options['parent_field_type']) || (isset($field->field_options['parent_field_type']) && $field->field_options['parent_field_type'] != 'arf_repeater')) ) {
                            ?>
                            <?php 
                                $field_name = $armainhelper->truncate($arfieldhelper->arf_execute_function($field->name,'strip_tags'), 60);
                                if($field->id !="" && $field_name=="" ){
    ...
    function get_shortcode_total_modal($form_id, $target_id = 'content', $type = 'all', $style = '', $is_total_field = false) {
        ...
        if (is_numeric($form_id)) {
            ...
            $exclude = "'divider','section','captcha','break','html'";
    ...
    function arf_getfields_basic_options_section() {
        ...
        $args = array(
            ...
            'number' => array(
                ...
                'numberrange' => 11,
                'numberrange_min_validation' => 12,
                'numbberrange_max_validation' => 13,
                'arf_prefix' => 14,
            ),
            'phone' => array(
                'labelname' => 1,
            ...
            'password' => array(
                ...
                'arf_prefix' => 14,
                'customwidth' => 15,
                'validate_password'=>16,
                'validate_password_msg'=>17
            ),
            'html' => array(
                'labelname' => 1,
                'htmlcontent' => 2
            ...
            'break' => array(
                ...
                'pagebreakstyle' => 6,
                'pagebreaktabsbar' => 7,
                'pagebreakstyle_position' => 8,
                'pagebreakclickable' => 9,
                'pagebreakstylewizardthemes' => 10,
            ),
            'scale' => array(
                'labelname' => 1,
                'fielddescription' => 2,
            ...
            'arfslider' => array(
                ...
                'numberofsteps' => 5,
                'defaultvalue' => 6,
                'arf_range_selector' => 7,
                'arf_range_defaultvalue' => 8,
                'numberrange' => 9,
                'arf_slider_tooltip' => 10,
            ),
            'colorpicker' => array(
                'labelname' => 1,
                'fielddescription' => 2,
    ...
    function arf_replace_shortcodes($content = '', $entry = 0, $is_for_mail = false) {
        ...
        if ($matches and $matches[1]) {
            foreach ($matches[1] as $shortcode) {
                if ($shortcode) {
                    global $arffield,$wpdb,$MdlDb,$style_settings,$arformcontroller;
                    ...
                    if ($field) {
                        ...
                        if ($field and $field->type == 'file') {
                            ...
                            if ($is_for_mail) {
                                $replace_with = str_replace('|','<br>',$replace_with);
                            }
                        }
                        ...
                        if( $field && 'arf_repeater' == $field->type ){
                            $repeater_field_content = '';
                            
                            $get_all_inner_fields = $wpdb->get_results( $wpdb->prepare( "SELECT a.*, b.* FROM `".$MdlDb->fields."` a LEFT JOIN ".$MdlDb->entry_metas." b ON b.field_id = a.id WHERE ( field_options LIKE '%\"parent_field\":\"%d\"%'  OR field_options LIKE '%\"parent_field\":%d%' ) AND entry_id=%d GROUP BY a.id", $field->id, $field->id, $entry->id ), ARRAY_A );
			                $valign = "";
                            ...
                            if(!empty($get_all_inner_fields)) {
                                if( count( $get_all_inner_fields ) == 1 ){
                                    ...
                                    $repeater_field_content .= "<table cellspacing='0'><tr" . (($odd) ? $bg_color : $bg_color_alt) . "><th ".$valign.">$field->name</th><td><table class='arf_repeater_table' border='1' cellpadding='10' cellspacing='0' style='border-color:#ccc'><tbody>";
                                    $repeater_field_content .= $inner_field_content;
                                    $repeater_field_content .= "</tbody></table></td></tr></table>";
                                    
                                } else if( count( $get_all_inner_fields ) > 1 ) {
                                    
                                    $repeater_field_content .= "<table class='arf_repeater_table' border='1' cellpadding='10' cellspacing='0' style='border-color:#ccc'>";
                                            ...
                                            foreach( $get_all_inner_fields as $key => $inner_fields ){
                                                ...
                                                if(!empty($inner_fields['entry_value']) && strpos($inner_fields['entry_value'],"[ARF_JOIN]") != -1) {
                                                    $entry_val = !empty($inner_fields['entry_value']) ? explode("[ARF_JOIN]", $inner_fields['entry_value']) : array(); 
                                                    $has_separate_value = false;
                                                    if( in_array( $inner_fields['type'], array('checkbox', 'radio', 'select', 'arf_multiselect', 'arf_autocomplete') ) ){
                                                        $field_opts = json_decode( $inner_fields['field_options'], true );
                                                        if( isset( $field_opts['separate_value'] ) && '1' == $field_opts['separate_value'] ){
                                                            $has_separate_value = true;
                                                        }
                                        ...
                                        $repeater_field_content .= "<tbody>";
                                            $i = 1;
                                            foreach( $fields_arr as $k => $frr ){
                                                foreach( $get_all_inner_fields as $inner_fields ){
                                                    $repeater_field_content .= "<tr" . (($odd) ? $bg_color : $bg_color_alt) . ">";
                                                    $repeater_field_content .= "<td ".$valign." style='width:35%'>" . $inner_fields['name'] . " [".$i."] </td>";
                                                    $repeater_field_content .= "<td>" . $frr[$inner_fields['field_id']] . "</td>";
                                                    $repeater_field_content .= "</tr>";
                                                }
                                                $i++;
                                            }
                                        $repeater_field_content .= "</tbody>";
                                    $repeater_field_content .= "</table>";
                                }
                                $replace_with = $repeater_field_content;
                            }
                        }
    ...
    function changeoptionorder($field) {
        ...
        if ($field['type'] == 'radio' || $field['type'] == 'checkbox' || $field['type'] == 'select' || $field['type'] == 'arf_multiselect' || $field['type'] == ARF_AUTOCOMPLETE_SLUG) {
    ...
    function get_form_pagebreak_fields($form_id, $form_key, $values) {
        ...
        $page_num = wp_cache_get('arf_total_breaks_'.$form_id);        
        if( false === $page_num ){
            ...
            wp_cache_set('arf_total_breaks_'.$form_id, $page_num);
        }
    ...
    function arf_replace_inline_field( $content ){
        ...
        if( !empty( $matches[0] ) ){
            foreach( $matches[0] as $shortcode ){
                ...
                $content = str_replace( $shortcode, '<span class="inline_field_value-'.end($splited_data).'"></span>', $content );
            }
        }
    ...
    function arf_replace_runningtotal_shortcode_exp($content, $field_ref, $preview, $all_fields ) {
        $tagregexp = '';
    ...
    function post_validation_filed_display( $field, $allfield, $posted_item_fields, $arf_conditional_logic_rules = array() ){
        $style = "";
        $hidden_fields = array();
    ...
    function arf_add_material_style_block($field, $frm_css, $display, $arf_main_label_cls, $arf_column_classes) {
        if ($frm_css['arfinputstyle'] == 'material') {
            ...
            <?php } else { ?>
                ...
                <?php } ?>
            <?php }
            $arf_input_style_label_position = array('checkbox','radio','scale','arf_smiley','arf_switch', 'arf_repeat', 'html','arfslider','slider','hidden','colorpicker','imagecontrol','like','file','break','divider','section','captcha');
    ...
    function arf_add_material_outlined_style_block( $field, $frm_css, $display, $arf_main_label_cls, $arf_column_classes ){
        if ($frm_css['arfinputstyle'] == 'material_outlined') {
            $arf_disply_required_field = true;
            $arf_disply_required_field = apply_filters('arf_disply_required_field_outside', $arf_disply_required_field, $field);
            $is_required_field = false;
            if ($display['required'] and $field['type'] != 'arfslider' && $field['type'] != 'imagecontrol' && $arf_disply_required_field) {
            ...
            <?php
                ...
                $active_cls = '';
            ?>
                <div class="arf_material_outliner <?php echo $active_cls; ?>">
                    <div class="arf_material_outliner_prefix"></div>
                    <div class="arf_material_outliner_notch">
                        <label class="arf_main_label <?php echo $arf_main_label_cls; ?>" id="field_<?php echo $field['id']; ?>">
                            <span class="arfeditorfieldopt_label arf_edit_in_place">
                                <input type="text" class="arf_edit_in_place_input inplace_field" data-ajax="false" data-field-opt-change="true" data-field-opt-key='name' value="<?php echo htmlspecialchars($field['name']); ?>" data-field-id="<?php echo $field['id']; ?>" />
                            </span>
                    ...
                    <div class="arf_material_outliner_suffix"></div>
                </div>
    ...
    function arf_add_blank_msg_from_globalsetting($form_field_json,$arfinputstyle){
        ...
        if( !isset($form_field_json) || empty($form_field_json) ){
            $form_field_json = $wp_filesystem->get_contents(VIEWS_PATH.'/arf_editor_data.json');
