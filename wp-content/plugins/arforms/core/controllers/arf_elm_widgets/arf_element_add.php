	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'ARForms',
				'options' => [
					'ARForms' => esc_html__( 'Internal', 'ARForms' ),
					'ARForms_popup' =>esc_html__( 'Modal (popup) Window', 'ARForms' ),
				],
				'label_block' => true,
		...
		$this->add_control(
			...
			[
					...
					'label_block' => true,
					'default' => '',
					'condition' => ['arf_frm_type' => 'ARForms_popup', 'arf_click_type' => 'image'],
					'classes'=>'arf_click_type_el',
			]
		);
		$this->add_control(
			'arf_image_height',
			[
				'label' => esc_html__('Image Height', 'ARForms'),
				'type' => Controls_Manager::TEXT,
				'label_block' => true,
				'default' => 'auto',
				'condition' => ['arf_frm_type' => 'ARForms_popup', 'arf_click_type' => 'image'],
		...
		$this->add_control(
			'arf_image_width',
			[
				'label' => esc_html__('Image Width', 'ARForms'),
				'type' => Controls_Manager::TEXT,
				'label_block' => true,
				'default' => 'auto',
				'condition' => ['arf_frm_type' => 'ARForms_popup', 'arf_click_type' => 'image'],
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Overlay Background Color :', 'ARForms' ),
				'type' => Controls_Manager::COLOR,
				'label_block' => false,
				'default' =>'#000000',
				'condition'=>['arf_frm_type' => 'ARForms_popup','arf_click_type'=>['link','button']],
		...
		$this->add_control(
			'arf_hide_popup_for_loggedin_user_input',
			[
				...
				'return_value' => 'yes',
				'condition' => ['arf_frm_type' => 'ARForms_popup'],
				'classes' => 'arf_hide_popup_for_loggedin_user',
			]
		);
		...
		echo '<div class="arf_select">';
			...
			if(isset($settings['arf_select']) && $settings['arf_select']=="Please select a valid form"){
				...
				if (isset($settings['arf_model_trigger_type']) && $settings['arf_model_trigger_type'] == 'onclick' && isset($settings['arf_click_type']) && $settings['arf_click_type'] == 'sticky') {
	               echo '[ARForms_popup '.$settings['arf_select'].' desc="'.$settings['arf_popup_label'].'" type="'.$settings['arf_click_type'].'" position="'.$settings['arf_link_position'].'" height="'.$settings['arf_height'].'" width="'.$settings['arf_width'].'" bgcolor="'.$settings['arf_btn_back_color'].'" txtcolor="'.$settings['arf_btn_text_color'].'" hide_popup_for_loggedin_user="'.$settings['arf_hide_popup_for_loggedin_user_input'].'"]';
	            } 
	            if (isset($settings['arf_model_trigger_type']) && $settings['arf_model_trigger_type'] == 'onclick' && isset($settings['arf_click_type']) && $settings['arf_click_type'] == 'fly'){
	                echo '[ARForms_popup '.$settings['arf_select'].' desc="'.$settings['arf_popup_label'].'" type="'.$settings['arf_click_type'].'" position="'.$settings['arf_fly_link_position'].'" width="'.$settings['arf_width'].'" angle="'.$settings['arf_btn_angle'].'" bgcolor="'.$settings['arf_btn_back_color'].'" txtcolor="'.$settings['arf_btn_text_color'].'" hide_popup_for_loggedin_user="'.$settings['arf_hide_popup_for_loggedin_user_input'].'"]';
	            }
	            if (isset($settings['arf_model_trigger_type']) && $settings['arf_model_trigger_type'] == 'onclick' && isset($settings['arf_click_type']) && $settings['arf_click_type'] == 'image'){
	                echo '[ARForms_popup '.$settings['arf_select'].' desc="'.$settings['arf_popup_label'].'" type="'.$settings['arf_click_type'].'" arf_img_url="'.$settings['arf_image_url'].'" width="'.$settings['arf_width'].'" arf_img_height="'.$settings['arf_image_height'].'" arf_img_width="'.$settings['arf_image_width'].'" hide_popup_for_loggedin_user="'.$settings['arf_hide_popup_for_loggedin_user_input'].'"]';
	            }
	            ...
	            if (isset($settings['arf_model_trigger_type']) && $settings['arf_model_trigger_type'] == 'on_exit') {
	                ...
	                echo '[ARForms_popup '.$settings['arf_select'].' type="'.$settings['arf_model_trigger_type'].'" width="'.$settings['arf_width'].'" modaleffect="'.$settings['arf_animation_effect'].'" is_fullscreen="'.$settings['arf_show_full_screen_popup'].'" is_close_link="'.$settings['arf_show_cl_btn'].'" modal_bgcolor="'.$settings['arf_back_color'].'" hide_popup_for_loggedin_user="'.$settings['arf_hide_popup_for_loggedin_user_input'].'"]';

	            }
	            if(isset($settings['arf_model_trigger_type']) && $settings['arf_model_trigger_type'] == 'on_idle'){

	               echo '[ARForms_popup '.$settings['arf_select'].' type="'.$settings['arf_model_trigger_type'].'" width="'.$settings['arf_width'].'" modaleffect="'.$settings['arf_animation_effect'].'" is_fullscreen="'.$settings['arf_show_full_screen_popup'].'" inactive_min="'.$settings['arf_popup_after_user_inactive'].'" overlay="'.$settings['arf_back_overlay'].'" is_close_link="'.$settings['arf_show_cl_btn'].'" modal_bgcolor="'.$settings['arf_back_color'].'" hide_popup_for_loggedin_user="'.$settings['arf_hide_popup_for_loggedin_user_input'].'"]';
	            ...
	            if(isset($settings['arf_model_trigger_type']) && $settings['arf_model_trigger_type'] == 'onclick' && isset($settings['arf_click_type']) && $settings['arf_click_type']=='link'){
					echo '[ARForms_popup '.$settings['arf_select'].' desc="'.$settings['arf_popup_label'].'" type="'.$settings['arf_click_type'].'" width="'.$settings['arf_width'].'" modaleffect="'.$settings['arf_click_animation_effect'].'"  is_fullscreen="'.$settings['arf_click_show_full_screen_popup'].'" overlay="'.$settings['arf_click_back_overlay'].'" is_close_link="'.$settings['arf_click_show_cl_btn'].'" modal_bgcolor="'.$settings['arf_click_back_color'].'" hide_popup_for_loggedin_user="'.$settings['arf_hide_popup_for_loggedin_user_input'].'"]';
				}		
			}else{
		...
		<div class="arf_select">
			...
			<# }else if(settings.arf_frm_type=='ARForms_popup'){ #>
					<# if (settings.arf_model_trigger_type=='onclick' && settings.arf_click_type=='sticky') { #>
					   [ARForms_popup {{{ settings.arf_select}}} desc="{{{ settings.arf_popup_label}}}" type="{{{ settings.arf_click_type}}}" position="{{{ settings.arf_link_position}}}" height="{{{ settings.arf_height}}}" width="{{{ settings.arf_width}}}" bgcolor="{{{ settings.arf_btn_back_color}}}" txtcolor="{{{ settings.arf_btn_text_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
					<# if(settings.arf_model_trigger_type=='onclick' && settings.arf_click_type=='fly'){ #>
					    [ARForms_popup {{{ settings.arf_select}}} desc="{{{ settings.arf_popup_label}}}" type="{{{ settings.arf_click_type}}}" position="{{{ settings.arf_fly_link_position}}}" height="{{{ settings.arf_height}}}" width="{{{ settings.arf_width}}}" angle="{{{settings.arf_btn_angle}}}" bgcolor="{{{ settings.arf_btn_back_color}}}" txtcolor="{{{ settings.arf_btn_text_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
					<# if(settings.arf_model_trigger_type=='onclick' && settings.arf_click_type=='image'){ #>
					    [ARForms_popup {{{ settings.arf_select}}} desc="{{{ settings.arf_popup_label}}}" type="{{{ settings.arf_click_type}}}"  arf_img_url="{{{ settings.arf_image_url}}}" width="{{{ settings.arf_width}}}" arf_img_height="{{{ settings.arf_image_height}}}" arf_img_width="{{{settings.arf_image_width}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
					<# if(settings.arf_model_trigger_type=='onclick' && settings.arf_click_type=='button'){ #>
					    [ARForms_popup {{{ settings.arf_select}}} desc="{{{ settings.arf_popup_label}}}" type="{{{ settings.arf_click_type}}}" width="{{{ settings.arf_width}}}" modaleffect="{{{ settings.arf_click_animation_effect}}}" is_fullscreen="{{{ settings.arf_click_show_full_screen_popup}}}" overlay="{{{ settings.arf_back_overlay}}}" is_close_link="{{{ settings.arf_click_show_cl_btn}}}" bgcolor="{{{ settings.arf_btn_back_color}}}" txtcolor="{{{ settings.arf_btn_text_color}}}" modal_bgcolor="{{{ settings.arf_back_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
					<# if(settings.arf_model_trigger_type=='onload'){ #>    
					    [ARForms_popup {{{ settings.arf_select}}} type="{{{ settings.arf_model_trigger_type}}}" width="{{{ settings.arf_width}}}" modaleffect="{{{ settings.arf_animation_effect}}}" is_fullscreen="{{{ settings.arf_show_full_screen_popup}}}" overlay="{{{ settings.arf_back_overlay}}}" is_close_link="{{{ settings.arf_show_cl_btn}}}" modal_bgcolor="{{{ settings.arf_back_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}" ]
					<# } #>
					<# if(settings.arf_model_trigger_type=='scroll'){ #>    
					    [ARForms_popup {{{ settings.arf_select}}} type="{{{ settings.arf_model_trigger_type}}}" width="{{{ settings.arf_width}}}" modaleffect="{{{ settings.arf_animation_effect}}}" is_fullscreen="{{{ settings.arf_show_full_screen_popup}}}" on_scroll="{{{ settings.arf_popup_on_scoll_position}}}" overlay="{{{ settings.arf_back_overlay}}}" is_close_link="{{{ settings.arf_show_cl_btn}}}" modal_bgcolor="{{{ settings.arf_back_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
					...
					<# if(settings.arf_model_trigger_type=='on_exit'){ #>    
					    [ARForms_popup {{{ settings.arf_select}}} type="{{{ settings.arf_model_trigger_type}}}" width="{{{ settings.arf_width}}}" modaleffect="{{{ settings.arf_animation_effect}}}" is_fullscreen="{{{ settings.arf_show_full_screen_popup}}}" is_close_link="{{{ settings.arf_show_cl_btn}}}" modal_bgcolor="{{{ settings.arf_back_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
					...
					<# if(settings.arf_model_trigger_type=='onclick' && settings.arf_click_type=='link'){ #>
						[ARForms_popup {{{ settings.arf_select}}} desc="{{{ settings.arf_popup_label}}}" type="{{{ settings.arf_click_type}}}" width="{{{ settings.arf_width}}}" modaleffect="{{{ settings.arf_click_animation_effect}}}"  is_fullscreen="{{{ settings.arf_click_show_full_screen_popup}}}" overlay="{{{ settings.arf_back_overlay}}}" is_close_link="{{{ settings.arf_click_show_cl_btn}}}" modal_bgcolor="{{{ settings.arf_back_color}}}" hide_popup_for_loggedin_user="{{{ settings.arf_hide_popup_for_loggedin_user_input}}}"]
					<# } #>
			<# }else{ #>
