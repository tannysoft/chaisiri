...
class ARForms_Popup{
	...
	function __construct(){
		...
		add_action('arfbeforedestroyform', array( $this, 'arfdelete_popup_form'), 11, 1);

	}

	function arfdelete_popup_form($form_id) {
		global $wpdb, $db_record,$MdlDb,$arf_paypal;

		if (!$form_id)
            return;

        $form_data = $wpdb->get_results($wpdb->prepare("SELECT form_id FROM " . $MdlDb->form_popup ." WHERE form_id = %d", $form_id));
        if (count($form_data) > 0) {
    		$res = $wpdb->query($wpdb->prepare("DELETE FROM " . $MdlDb->form_popup . " WHERE form_id = %d", $form_id));
        }
	}
			...
			foreach ( $popup_ids as $key => $popup ) {
				...
				if ( 0==$popup['popup_option']['arf_hide_popup_in_404'] && 1==$popup['status'] && 1==$popup['popup_option']['arf_display_popup_in_posts'] ) {
					...
					switch($post_link_type){
						...
						break;
						case 'onload':
							$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $post_link_type . "' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' site_wide='yes' ]";
						break;
						case 'scroll':
							$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $post_link_type . "' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' on_scroll='".$post_open_scroll."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' site_wide='yes' ]";
						break;
						case 'timer':
							$form_shortcode = "[ARForms_popup id=". $form_id ." on_delay='".$post_open_delay."' type='".$post_link_type."' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' site_wide='yes']";
						break;
						case 'on_exit':
							$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$post_link_type."' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' site_wide='yes' ]";
						break;
						case 'on_idle':
							$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$post_link_type."' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' inactive_min='".$post_inact_time."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' site_wide='yes']";
						break;
					}
					...
					switch($page_link_type){
						...
						break;
						case 'onload':
							$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $page_link_type . "' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' site_wide='yes' ]";
						break;
						case 'scroll':
							$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $page_link_type . "' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' on_scroll='".$page_open_scroll."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' site_wide='yes' ]";
						break;
						case 'timer':
							$form_shortcode = "[ARForms_popup id=". $form_id ." on_delay='".$page_open_delay."' type='".$page_link_type."' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' site_wide='yes']";
						break;
						case 'on_exit':
							$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$page_link_type."' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' site_wide='yes' ]";
						break;
						case 'on_idle':
							$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$page_link_type."' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' inactive_min='".$page_inact_time."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' site_wide='yes']";
						break;
					}
	...
	function arf_form_popup_function($content){
		...
		global $wpdb,$arfform,$post, $MdlDb, $arf_popup_forms;

		$popup_type = "";
		if( isset( $post ) && isset( $post->post_type ) && 'post'==$post->post_type ){
			$popup_type = "post";
		}
		if( isset( $post ) && isset( $post->post_type ) && 'page'==$post->post_type ){
			$popup_type = "page";
		}
		...
		foreach ($popup_ids as $key => $popup) {
			...
			if ( ( (is_search() && 0==$popup['popup_option']['arf_hide_popup_in_search_result']) || (is_front_page() && 0==$popup['popup_option']['arf_hide_popup_in_front']) || (is_archive() && 0==$popup['popup_option']['arf_hide_popup_in_archive']) || 'post'==$post->post_type ) && 1==$popup['status'] && 1==$popup['popup_option']['arf_display_popup_in_posts'] && !in_array($post->ID,$exclude_posts) ) {
				...
				$post_model_effect = $popup['popup_option']['arf_popup_post_model_effect'];
				$post_inact_time = $popup['popup_option']['arf_popup_post_inactive_time'];
				$hide_popup_for_loggedin_user = isset($popup['popup_option']['arf_hide_popup_for_loggedin_user_input']) ? $popup['popup_option']['arf_hide_popup_for_loggedin_user_input'] : "no";
				...
				switch($post_link_type){
					case 'onclick':
						if( $post_click_type == 'sticky' ){
							$form_shortcode = "[ARForms_popup id=".$form_id." desc='".$post_link_caption."' type='sticky' position='".$post_link_position."' height='".$post_model_height."' width='".$post_model_width."' bgcolor='".$post_model_btn_bgcolor."' txtcolor='".$post_model_btn_txt_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."']";
						} else if( $post_click_type == 'fly' ){
							$form_shortcode = "[ARForms_popup id=".$form_id." desc='".$post_link_caption."' type='fly' position='".$post_link_position_fly."' height='".$post_model_height."' width='".$post_model_width."' bgcolor='".$post_model_btn_bgcolor."' txtcolor='".$post_model_btn_txt_color."' angle='".$post_link_button_angle."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' ]";
						}
					break;
					case 'onload':
						$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $post_link_type . "' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'scroll':
						$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $post_link_type . "' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' on_scroll='".$post_open_scroll."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'timer':
						$form_shortcode = "[ARForms_popup id=". $form_id ." on_delay='".$post_open_delay."' type='".$post_link_type."' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes']";
					break;
					case 'on_exit':
						$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$post_link_type."' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'on_idle':
						$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$post_link_type."' width='".$post_model_width."' modaleffect='".$post_model_effect."' is_fullscreen='".$post_show_full_screen."' inactive_min='".$post_inact_time."' overlay='".$post_overlay."' is_close_link='".$post_show_close."' modal_bgcolor='".$post_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes']";
					break;
				}
				...
				switch($page_link_type){
					case 'onclick':
						if( $page_click_type == 'sticky' ){
							$form_shortcode = "[ARForms_popup id=".$form_id." desc='".$page_link_caption."' type='sticky' position='".$page_link_position."' height='".$page_model_height."' width='".$page_model_width."' bgcolor='".$page_model_btn_bgcolor."' txtcolor='".$page_model_btn_txt_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
						} else if( $page_click_type == 'fly' ){
							$form_shortcode = "[ARForms_popup id=".$form_id." desc='".$page_link_caption."' type='fly' position='".$page_link_position_fly."' height='".$page_model_height."' width='".$page_model_width."' bgcolor='".$page_model_btn_bgcolor."' txtcolor='".$page_model_btn_txt_color."' angle='".$page_link_button_angle."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
						}
					break;
					case 'onload':
						$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $page_link_type . "' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'scroll':
						$form_shortcode = "[ARForms_popup id=" . $form_id . " type='" . $page_link_type . "' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' on_scroll='".$page_open_scroll."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'timer':
						$form_shortcode = "[ARForms_popup id=". $form_id ." on_delay='".$page_open_delay."' type='".$page_link_type."' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'on_exit':
						$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$page_link_type."' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes' ]";
					break;
					case 'on_idle':
						$form_shortcode = "[ARForms_popup id=". $form_id ." type='".$page_link_type."' width='".$page_model_width."' modaleffect='".$page_model_effect."' is_fullscreen='".$page_show_full_screen."' inactive_min='".$page_inact_time."' overlay='".$page_overlay."' is_close_link='".$page_show_close."' modal_bgcolor='".$page_overlay_color."' hide_popup_for_loggedin_user='".$hide_popup_for_loggedin_user."' site_wide='yes']";
					break;
				}
	...
	function save_popup_data(){
		...
		if(isset($_POST['action']) && 'save_popup_data'==$_POST['action']){
			...
			if(isset($_POST['data']) && !empty($_POST['data'])){
				...
				$popup_opt['arf_hide_popup_for_loggedin_user_input'] = isset($opt_arr['arf_hide_popup_for_loggedin_user_input']) ? arf_sanitize_value($opt_arr['arf_hide_popup_for_loggedin_user_input']) : 'no';
	...
	function enqueue_chosen_style_and_script(){
		...
		if(isset($_REQUEST['page']) && 'ARForms-popups'==$_REQUEST['page']){
			...
			';
			wp_add_inline_script('arforms_admin', $popup_internal_script);	
		}
	...
	function arf_load_popup_list(){
		...
		if($count > 0){
			foreach ($popup_result as $key => $popup_data) {
				...
				$popup_opts = arf_json_decode( $popup_data->popup_option );
				$popup_datatype = $popup_data->popup_type;
				if( 'post' == $popup_datatype || 'page' == $popup_datatype ){
					$trigger_type = $this->arf_get_popup_trigger_type($popup_opts,$popup_datatype);
					$popup_data->popup_type = $popup_datatype.' ('.$trigger_type.')';
				} else {
					$trigger_type = $this->arf_get_popup_trigger_type($popup_opts,'both');
					$triggers = explode( '||', $trigger_type );
					$popup_data->popup_type = esc_html__('Post','ARForms').' ('.$triggers[0].'), '.esc_html__('Page','ARForms').' ('.$triggers[1].')';
				}
	...
	function arf_get_popup_trigger_type( $popup_opts, $popup_type ){


		$trigger_type = '';

		if( 'post' == $popup_type || 'page' == $popup_type ){
			if( 'post' == $popup_type ){
				$link_type = $popup_opts->arf_popup_post_link_type;
			} else {
				$link_type = $popup_opts->arf_popup_page_link_type;
			}
			if( 'onclick' == $link_type ){
				if( 'post' == $popup_type ){
					if( 'sticky' == $popup_opts->arf_popup_post_click_type ){
						$trigger_type = esc_html__('Sticky','ARForms');
					} else if( 'fly' == $popup_opts->arf_popup_post_click_type ) {
						$trigger_type = esc_html__('Fly','ARForms');
					}
				} else {
					if( 'sticky' == $popup_opts->arf_popup_page_click_type ){
						$trigger_type = esc_html__('Sticky','ARForms');
					} else if( 'fly' == $popup_opts->arf_popup_page_click_type ) {
						$trigger_type = esc_html__('Fly','ARForms');
					}
				}
			} else if( 'onload' == $link_type ){
				$trigger_type = esc_html__('On Page Load','ARForms');
			} else if( 'scroll' == $link_type ){
				$trigger_type = esc_html__('On Page Scroll','ARForms');
			} else if( 'timer' == $link_type ){
				$trigger_type = esc_html__('On Timer','ARForms');
			} else if( 'on_exit' == $link_type ){
				$trigger_type = esc_html__('On Exit Intent','ARForms');
			} else if( 'on_idle' == $link_type ){
				$trigger_type = esc_html__('On Idle','ARForms');
			}
		} else if( $popup_type == 'both' ){
			$post_trigger_type = $this->arf_get_popup_trigger_type( $popup_opts, 'post' );
			$page_trigger_type = $this->arf_get_popup_trigger_type( $popup_opts, 'page' );
			return $post_trigger_type.'||'.$page_trigger_type;
		}
