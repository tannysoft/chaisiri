   ...
   function __construct() {
    ...
    function createfield($field_data) {
        ...
        switch ($field_data['type']) {
            ...
            case 'divider':
            case 'section':
                $field_data['name'] = addslashes(esc_html__('Section Title', 'ARForms'));
    ...
    function getOne($id) {
        ...
        if ($results) {
            ...
            $results->rows = !empty( $results->field_options['rows'] ) ? maybe_unserialize( $results->field_options['rows'] ) : array();
            ...
            if (is_array($results)) {
                foreach ($results as $r_key => $result) {
                    ...
                    if( is_array($result->field_options) ){
                        ...
                        $results[$r_key]->field_options = arf_json_decode($result->field_options, true);
                        if (json_last_error() != JSON_ERROR_NONE) {
                    ...
                    if( is_array($result->options) ){
                        ...
                        $results[$r_key]->options = arf_json_decode($result->options, true);
                        if (json_last_error() != JSON_ERROR_NONE) {
