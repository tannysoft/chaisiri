...
class arformmodel {
    ...
    function create($values) {
        ...
        $new_values['is_imported_from_lite'] = isset($values['is_imported_from_lite']) ? $values['is_imported_from_lite'] : 0; 
        if( isset($values['id']) ){
    ...
    function update($id, $values, $create_link = false, $is_ref_form = 0) {
        ...
        if ($all_fields and ( isset($values['options']) or isset($values['item_meta']) or isset($values['field_options']))) {
            ...
            foreach ($all_fields as $fid) {
                ...
                if (!in_array($fid->id, $existing_keys))
                 ...
                 if($fid->type=='break'){
                    ...
                    if (isset($values['pagebreakclickable']) && $values['pagebreakclickable'] == 1) {
                        $arf_page_break_clickable = 1;
                    }

                    if( isset($values['field_options']['page_break_wizard_theme_' . $fid->id]) && $values['field_options']['page_break_wizard_theme_' . $fid->id] =='top'){
                        $arf_page_break_wizard_theme = $values['field_options']['page_break_wizard_theme_' . $fid->id];
                    }
                 }
            ...
            $options['arf_page_break_wizard_theme_style'] = $arf_page_break_wizard_theme;
        ...
        $new_values2['arfpagebreakinheritanimation'] = isset($_REQUEST['arfpbian'])? arf_sanitize_value($_REQUEST['arfpbian']):'';
        ...
        $new_values2['arffieldanimationdurationsetting'] = isset($_REQUEST['arfandus'])?arf_sanitize_value($_REQUEST['arfandus']):'';
        $new_values2['arfpbfieldanimationdurationsetting'] = isset($_REQUEST['arfpbandus'])?arf_sanitize_value($_REQUEST['arfpbandus']):'';
        
        $new_values2['arffieldanimationdelaysetting'] = isset($_REQUEST['arfandls'])?arf_sanitize_value($_REQUEST['arfandls']):'';
        $new_values2['arfpbfieldanimationdelaysetting'] = isset($_REQUEST['arfpbandls'])?arf_sanitize_value($_REQUEST['arfpbandls']):'';
        ...
        $new_values2['arfsubmitboxxoffsetsetting'] = isset($_REQUEST['arfsbxos'])?arf_sanitize_value($_REQUEST['arfsbxos']):'';
       

        $new_values2['arfsubmitboxyoffsetsetting'] = isset($_REQUEST['arfsbyos'])?arf_sanitize_value($_REQUEST['arfsbyos']):'';

        $new_values2['arfsubmitboxblursetting'] = isset($_REQUEST['arfsbbs'])?arf_sanitize_value($_REQUEST['arfsbbs']):'';

        $new_values2['arfsubmitboxshadowsetting'] = isset($_REQUEST['arfsbsps'])?arf_sanitize_value($_REQUEST['arfsbsps']):'';
        ...
        $new_values2['text_color_pg_break_style3'] = isset($_REQUEST['arfftcpbs3'])?arf_sanitize_value($_REQUEST['arfftcpbs3']):"";
        ...
        $new_values2['arffieldanimationstyle'] = isset($_REQUEST['arffans'])?arf_sanitize_value($_REQUEST['arffans']):'';
        $new_values2['arfpbfieldanimationstyle'] = isset($_REQUEST['arfpbfans'])?arf_sanitize_value($_REQUEST['arfpbfans']):'';
        ...
        $new_values2['arftooltipposition'] = isset($_REQUEST['arftippos'])?arf_sanitize_value($_REQUEST['arftippos']):"";
        ...
        if ($form_css_res) {
            foreach ($form_css_res as $refform) {
                ...
                if (isset($rformid) && $rformid > 0 && $rformid != "") {
                    ...
                    if( file_exists( $maincss_path . 'maincss_materialize_outlined_' . $rformid . '.css' ) ){
                        unlink( $maincss_path . 'maincss_materialize_outlined_' . $rformid . '.css' );
                    }
    ...
    function getOne($id, $blog_id = false) {
        ...
        $results = wp_cache_get( $id, 'arfform' );
        ...
        if (isset($results->options)) {
            ...
            wp_cache_set($id, $results, 'arfform');
        }
    ...
    function getAll($where = array(), $order_by = '', $limit = '', $is_ref_form = 0) {
        ...
        if ($limit == ' LIMIT 1' or $limit == 1) {
            ...
            if (is_array($where)) {
                ...
                $results = wp_cache_get('arf_all_form_query');
                if( false == $results ){
                    $results = $wpdb->get_results($query);
                    wp_cache_set('arf_all_form_query', $results);
                }
            }
    ...
    function getAll_forms_addon($return_results = array(),$where = array(), $order_by = '', $limit = '', $is_ref_form = 0) {
        ...
        foreach ($results as $key => $value) {
            $arf_display_lite_forms = apply_filters( 'arf_display_lite_forms', false, $value );
            if( !empty( $value->arf_is_lite_form ) && 1 == $value->arf_is_lite_form && false == $arf_display_lite_forms ){
                continue;
            }
    ...
    function arf_select_db_data( $default_query = true, $custom_query = '', $tablename = '', $fields = '*', $where = '', $params = array(), $groupby = '', $orderby = '', $limit = '', $isCounter = false, $single = false, $return_type = 'OBJECT' ){
        ...
        if( $default_query ){
            ...
            $fields = apply_filters( 'arf_modify_query_fields', $fields, $tablename );

            $where = apply_filters( 'arf_modify_where_clause', $where, $tablename );

            $params = apply_filters( 'arf_modify_where_placeholder', $params, $tablename, $where );

            $sel_query = "SELECT " . $fields . " FROM " . $tablename ." " . $where . " " . $groupby . " " . $orderby . " " . $limit;

            if( !empty( $params ) ){

                array_unshift( $params , $sel_query );

                $build_query = call_user_func_array( array( $wpdb, 'prepare' ) , $params );

            } else {

                $build_query = $sel_query;
            }
        } else {
            $build_query = $custom_query;
        }
        ...
        if( $isCounter ){
            $db_result = $wpdb->get_var( $build_query );
        } else if( $single ){
            $db_result = $wpdb->get_row( $build_query, $return_type );
        } else {
            $db_result = $wpdb->get_results( $build_query, $return_type );
        }
